...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:output method="html" encoding="utf-8" standalone="yes" indent="yes" omit-xml-declaration="yes"/>
                ...
                <xsl:for-each select="//observation[session=$currentSession]">
                    ...
                    <li>
                        <div>
                            ...
                            <xsl:value-of select="@id"/>
                            <xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
                            <span><xsl:value-of select="$currentTarget/name"/></span>
                            <span>
                                <xsl:call-template name="formatDate">
                                        <xsl:with-param name="dateTime" select="begin" />
                                </xsl:call-template>
    ...
    <xsl:template match="observation">
        ...
        <div class="observation">
            ...
            <xsl:apply-templates select="$currentTarget" mode="observation"/>
    ...
    <xsl:template match="target" mode="observation">
        <div>
            <div class="target">
                ...
                <div><xsl:if test="count(constellation)>0">
                        ...
                        <span class="label">
                            <xsl:call-template name="language-text"><xsl:with-param name="text">target.constellation</xsl:with-param></xsl:call-template>
                        </span>
                        <span><xsl:value-of select="constellation"/></span>
                    </xsl:if>
                </div>
            </div>
    ...
    <xsl:template name="formatDateAsDay">
        <xsl:param name="dateTime" />
        <xsl:variable name="date" select="substring-before($dateTime, 'T')" />
        <xsl:variable name="year" select="substring-before($date, '-')" />
        <xsl:variable name="month" select="substring-before(substring-after($date, '-'), '-')" />
    ...
    <xsl:template match="target" mode="targetListBottom">
        ...
        <xsl:choose>
            <xsl:when test="@type='oal:PlanetTargetType' or @type='oal:MoonTargetType' or  @type='oal:SunTargetType'">
                <xsl:variable name="objectName"  select="name"/>
                <xsl:call-template name="language-text"><xsl:with-param name="text"><xsl:value-of select="$objectName"/></xsl:with-param></xsl:call-template>
                ( <xsl:value-of select="count(//observations/observation[target=$idTarget])"/> )
            </xsl:when>
        ...
        <div class="infoTarget">
             <xsl:for-each select="//observations/observation[target=$idTarget]">
             <xsl:sort select="begin"/>
    ...
    <xsl:template match="target" mode="linkToDigitalSurvey">
       <div>
       <xsl:variable name="aaaa"  select="document-uri()"/>
        <span>aaaaa:<xsl:value-of select="$aaaa"/></span>
        <ul>
        <li>
