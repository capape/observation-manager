...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:external="http://ExternalFunction.xalan-c++.xml.apache.org" exclude-result-prefixes="external">
				...
				<tr>
					...
					<td valign="top" width="20%">
						...
						<xsl:if test="count(constellation)>0">
							<b>Sterrenbeeld: </b> <xsl:value-of select="constellation"/>
						</xsl:if>
						<xsl:if test="count(constellation)=0">
							<b>Geen sterrenbeeld: </b>
						</xsl:if>
						...
						<table border="1" cellspacing="0" cellpadding="2" width="60%" style="font-size:14;font-family:Verdana,Arial">
							...
							<tr>
								...
								<td>
									...
									<xsl:choose>
										...
										<xsl:when test="@type='oal:deepSkyMS'">Meervoudig stersysteem</xsl:when>
							...
							<xsl:if test="contains(@type,'oal:deepSky')">
								...
								<xsl:if test="boolean(surfBr)">
									<tr>
										<td>Oppervlaktehelderheid:	<xsl:value-of select="surfBr"/> mags/sq.arcmin</td>
									</tr>
								</xsl:if>
							</xsl:if>
							...
							<xsl:if test="boolean(datasource)">
								<tr>
									<td>
										<b>Oorsprong: </b><xsl:value-of select="datasource"/></td>
								</tr>
							</xsl:if>							
	...
	<xsl:template name="formatHHMM">
		...
		<result><xsl:value-of select="$hrs"/>u <xsl:if test="$minutes &lt; 10">0</xsl:if><xsl:value-of select="$minutes"/>m <xsl:if test="$sec  &lt; 10">0</xsl:if><xsl:value-of select="$sec"/>s</result>
	...
	<xsl:template name="linkTop">
		...
		<xsl:text disable-output-escaping="yes">&lt;a href="#objectList"&gt; &gt;&gt; Objectenlijst &lt;&lt;&lt;/a&gt;</xsl:text>
