...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:external="http://ExternalFunction.xalan-c++.xml.apache.org" exclude-result-prefixes="external">
				...
				<tr>
					...
					<td valign="top" width="20%">
						...
						<xsl:if test="count(constellation)>0">
							<b>T\u00c3\u00a4hdist\u00c3\u00b6: </b> <xsl:value-of select="constellation"/>
						</xsl:if>
						<xsl:if test="count(constellation)=0">
							<b>Ei t\u00c3\u00a4hdist\u00c3\u00b6\u00c3\u00a4: </b>
						</xsl:if>
						...
						<table border="1" cellspacing="0" cellpadding="2" width="60%" style="font-size:14;font-family:Verdana,Arial">
							...
							<tr>
								...
								<td>
									...
									<xsl:choose>
										...
										<xsl:when test="@type='oal:deepSkyGC'">Pallomainen t\u00c3\u00a4htijoukko</xsl:when>
										<xsl:when test="@type='oal:deepSkyGN'">Emissiosumu</xsl:when>
										<xsl:when test="@type='oal:deepSkyOC'">Avoin t\u00c3\u00a4htijoukko</xsl:when>
										<xsl:when test="@type='oal:deepSkyPN'">Planetaarinen sumu</xsl:when>
										<xsl:when test="@type='oal:deepSkyQS'">Kvasaari</xsl:when>
										<xsl:when test="@type='oal:deepSkyDS'">Kaksoist\u00c3\u00a4hti</xsl:when>
										<xsl:when test="@type='oal:deepSkyDN'">Pime\u00c3\u00a4 sumu</xsl:when>
										<xsl:when test="@type='oal:deepSkyAS'">Asterismi</xsl:when>
										<xsl:when test="@type='oal:deepSkySC'">Star cloud</xsl:when>
										<xsl:when test="@type='oal:deepSkyMS'">Usean t\u00c3\u00a4hden systeemi</xsl:when>
										<xsl:when test="@type='oal:deepSkyCG'">Galaksijoukko</xsl:when>
										<xsl:when test="@type='oal:variableStarTargetType'">Muuttuva t\u00c3\u00a4hti</xsl:when>
							...
							<xsl:if test="contains(@type,'oal:deepSky')">
								...
								<xsl:if test="boolean(visMag)">
									<tr>
										<td>
											<b>Magnitudi (vis): </b> <xsl:value-of select="visMag"/> mag</td>
									</tr>
								</xsl:if>
								<xsl:if test="boolean(surfBr)">
									<tr>
										<td>Pintakirkkaus:	<xsl:value-of select="surfBr"/> mags/sq.arcmin</td>
									</tr>
								</xsl:if>
							</xsl:if>
							...
							<xsl:if test="boolean(datasource)">
								<tr>
									<td>
										<b>L\u00c3\u00a4hde: </b><xsl:value-of select="datasource"/></td>
								</tr>
							</xsl:if>							
	...
	<xsl:template match="/">
		...
		<HTML>
			...
			<HEAD>
				...
				<TITLE>Kohdeluettelo</TITLE>

			</HEAD>

			<BODY>

				<div align="center" style="font-size:24;font-family:Verdana,Arial;color:#0000C0">Objectlist</div>
	...
	<xsl:template name="linkTop">
		...
		<xsl:text disable-output-escaping="yes">&lt;a href="#objectList"&gt; &gt;&gt; Kohdeluettelo &lt;&lt;&lt;/a&gt;</xsl:text>
