...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:external="http://ExternalFunction.xalan-c++.xml.apache.org" exclude-result-prefixes="external">
				...
				<tr>
					...
					<td valign="top" width="20%">
						...
						<xsl:if test="count(constellation)>0">
							<b>Constela\ufffd\ufffdo: </b> <xsl:value-of select="constellation"/>
						</xsl:if>
						<xsl:if test="count(constellation)=0">
							<b>N\ufffdo faz parte de constela\ufffd\ufffdo: </b>
						</xsl:if>
						...
						<table border="1" cellspacing="0" cellpadding="2" width="60%" style="font-size:14;font-family:Verdana,Arial">
							...
							<tr>
								...
								<td>
									...
									<xsl:choose>
										...
										<xsl:when test="@type='oal:deepSkySC'">Nuvem Estelar</xsl:when>
										<xsl:when test="@type='oal:deepSkyMS'">Sistema de Estrelas M\ufffdltiplas</xsl:when>
										...
										<xsl:when test="@type='oal:MinorPlanetTargetType'">Planeta an\ufffdo</xsl:when>
							...
							<xsl:if test="contains(@type,'oal:deepSky')">
								...
								<xsl:if test="boolean(surfBr)">
									<tr>
										<td>Brilho na superf\ufffdcio:	<xsl:value-of select="surfBr"/> mags/sq.arcmin</td>
									</tr>
								</xsl:if>
							</xsl:if>
							...
							<xsl:if test="boolean(datasource)">
								<tr>
									<td>
										<b>Origem: </b><xsl:value-of select="datasource"/></td>
								</tr>
							</xsl:if>							
	...
	<xsl:template match="/">
		...
		<HTML>
			...
			<BODY>
				...
				<div align="center" style="font-size:24;font-family:Verdana,Arial;color:#0000C0">Lista de Objetos</div>
	...
	<xsl:template name="linkTop">
		...
		<xsl:text disable-output-escaping="yes">&lt;a href="#objectList"&gt; &gt;&gt; Objectlist &lt;&lt;&lt;/a&gt;</xsl:text>
