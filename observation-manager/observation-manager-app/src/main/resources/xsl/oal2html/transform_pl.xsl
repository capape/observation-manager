...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:external="http://ExternalFunction.xalan-c++.xml.apache.org" exclude-result-prefixes="external">
	...
	<xsl:template match="session">
		<p>
			...
			<b>Sesje: <xsl:value-of select="substring-before(begin, 'T')"/> o <xsl:value-of select="substring-after(begin, 'T')"/></b>
		...
		<table border="0" cellspacing="0" cellpadding="2" style="font-size:12;font-family:Verdana, Arial">
			<tr>
				<td>Początek:</td>
				<td>
					<xsl:value-of select="substring-before(begin, 'T')"/> o <xsl:value-of select="substring-after(begin, 'T')"/>
				</td>
			</tr>
			<tr>
				<td>Koniec:</td>
				<td>
					<xsl:value-of select="substring-before(end, 'T')"/> o <xsl:value-of select="substring-after(end, 'T')"/>
				</td>
			</tr>
		</table>
	...
	<xsl:template match="target">
		...
		<xsl:choose>
			...
			<xsl:when test="@type='oal:MinorPlanetTargetType'"> Planeta karlowata</xsl:when>
		...
		<xsl:if test="count(constellation)>0"> w <xsl:value-of select="constellation"/>
		...
		<table border="0" cellspacing="0" cellpadding="2" style="font-size:12;font-family:Verdana, Arial">
			...
			<xsl:if test="contains(@type,'oal:deepSky')">
				...
				<xsl:if test="boolean(visMag)">
					<tr>
						<td>Wielkość gwiazdowa(wizualna):</td>
						<td>
				...
				<xsl:if test="boolean(surfBr)">
					<tr>
						<td>Jasność powierzchniowa:</td>
						<td>
							<xsl:value-of select="surfBr"/> mag / (arcmin^2)</td>
					</tr>
				</xsl:if>
	...
	<xsl:template match="eyepiece">
		<p>
			...
			<xsl:if test="count(maxFocalLength)>0">
				...
				<b>Okular o zmiennej ogniskowej: </b>
		...
		<table border="0" cellspacing="0" cellpadding="2" style="font-size:12;font-family:Verdana, Arial">
			<xsl:if test="count(type)>0">
				<tr>
					...
					<td>
						<xsl:choose>
							<xsl:when test="type='other'">Inny</xsl:when>
							<xsl:when test="type='broad band'">Szerokopasmowy</xsl:when>
							<xsl:when test="type='narrow band'">Wąskopasmowy</xsl:when>
							<xsl:when test="type='O-III'">OIII</xsl:when>
							<xsl:when test="type='Solar'">Słoneczny</xsl:when>
				...
				<xsl:if test="count(color)>0">
					<tr>
						...
						<td>
						<xsl:choose>
							<xsl:when test="color='light red'">Jasnoczerwony</xsl:when>
							<xsl:when test="color='red'">Czerwony</xsl:when>
							<xsl:when test="color='deep red'">Ciemnoczerwony</xsl:when>
							<xsl:when test="color='orange'">Pomarańczowy</xsl:when>
							<xsl:when test="color='light yellow'">Jasnożólty</xsl:when>
							<xsl:when test="color='deep yellow'">Ciemnozólty</xsl:when>
			...
			<xsl:if test="contains(./@type,'findingsDeepSkyType') or contains(./@type,'findingsDeepSkyOCType') or contains(./@type,'findingsDeepSkyDSType')">
				...
				<li>Rating wizualny: 
						...
						<xsl:when test="contains(./@type,'findingsDeepSkyOCType')">
							<!-- open starcluster -->
							<xsl:choose>
								<xsl:when test="rating = '1'">Bardzo widoczna, ładna gromada, rzuca się w oczy</xsl:when>
								<xsl:when test="rating = '2'">Wyróżniajacą się gromada</xsl:when>
								<xsl:when test="rating = '3'">Wyraźnie widoczna gromada</xsl:when>
								<xsl:when test="rating = '4'">Gromada gwiazd, niezbyt widoczna</xsl:when>
								<xsl:when test="rating = '5'">Słabo widoczna gromada, łatwa do przeoczenia przy poszukiwaniu</xsl:when>
								<xsl:when test="rating = '6'">Widoczność wątpliwa, gestość gwiazd nie jest większa niż w otoczeniu</xsl:when>
								<xsl:when test="rating = '7'">Prawie nie ma gwiazd na tej pozycji</xsl:when>
							</xsl:choose>
						</xsl:when>
						...
						<xsl:when test="contains(./@type,'findingsDeepSkyDSType')">
							<!-- Double stars -->
							<xsl:choose>
								<xsl:when test="rating = '1'">Składniki gwiazdy podwójnej są rozdzielone</xsl:when>
								<xsl:when test="rating = '2'">Gwiazda podwójna jest widoczna jako "8"</xsl:when>
								<xsl:when test="rating = '3'">Składniki gwiazdy podwójnej nie są rozdzielone</xsl:when>
							</xsl:choose>
						</xsl:when>
						...
						<xsl:otherwise>
							<!-- other objecttype -->
							<xsl:choose>
								<xsl:when test="rating = '1'">Obiekt dobrze widoczny w okularze</xsl:when>
								<xsl:when test="rating = '2'">Dobrze widoczny bezpośrednio</xsl:when>
								<xsl:when test="rating = '3'">Widoczny bezpośrednio</xsl:when>
								<xsl:when test="rating = '4'">Widoczny tylko przy zerkaniu</xsl:when>
								<xsl:when test="rating = '5'">Obiekt ledwo widoczny przy zerkaniu</xsl:when>
								<xsl:when test="rating = '6'">Jest wątpliwe, czy obiekt byl widziany</xsl:when>
								<xsl:when test="rating = '7'">Obiekt niewidoczny</xsl:when>
							</xsl:choose>
						</xsl:otherwise>
					</xsl:choose>
				</li>
				...
				<xsl:if test="./@stellar='true'">
					<li>Wygląda jak gwiazda<br/>
					</li>
				</xsl:if>


				<xsl:if test="./@resolved='true'">
					<li>Jest rozdzielona<br/>
					</li>
				</xsl:if>


				<xsl:if test="./@mottled='true'">
					<li>Jest wzorzysta<br/>
					</li>
				</xsl:if>
	...
	<xsl:template match="/">
		<HTML>
			<HEAD>
				<TITLE>Dziennik obserwacji</TITLE>
			</HEAD>
			<BODY>
				<div align="center" style="font-size:24;font-family:Verdana,Arial;color:#0000C0">Dziennik obserwacji</div>
				...
				<td valign="top">
					<table border="1" cellspacing="0" cellpadding="2" width="400" style="font-size:14;font-family:Verdana,Arial">
						...
						<tr>
							<td>
								<xsl:choose>
									<xsl:when test="count(end) = 1">Poczatek</xsl:when>
									<xsl:otherwise>Czas</xsl:otherwise>
								</xsl:choose>
							</td>
							<td>
								<xsl:value-of select="substring-before(begin, 'T')"/> o <xsl:value-of select="substring-after(begin, 'T')"/>
						...
						<xsl:if test="count(seeing) = 1">
							<tr>
								...
								<td>
									...
									<xsl:choose>
										<xsl:when test="seeing = 1"> (bardzo dobry)</xsl:when>
										<xsl:when test="seeing = 2"> (dobry)</xsl:when>
										<xsl:when test="seeing = 3"> (umiarkowany)</xsl:when>
										<xsl:when test="seeing = 4"> (słaby)</xsl:when>
										<xsl:when test="seeing = 5"> (bardzo słaby)</xsl:when>
									</xsl:choose>
								</td>
							</tr>
						</xsl:if>
						...
						<xsl:if test="count(filter) = 1">
							<tr>
								...
								<td>
									...
									<xsl:text disable-output-escaping="yes">&lt;/a&gt; </xsl:text>
									<xsl:choose>
										<xsl:when test="key('filterKey', filter)/type='other'">Inny</xsl:when>
										<xsl:when test="key('filterKey', filter)/type='broad band'">Szerokopasmowy</xsl:when>
										<xsl:when test="key('filterKey', filter)/type='narrow band'">Waskopasmowy</xsl:when>
										<xsl:when test="key('filterKey', filter)/type='O-III'">OIII</xsl:when>
										<xsl:when test="key('filterKey', filter)/type='solar'">Sloneczny</xsl:when>
						...
						<xsl:if test="count(session) = 1">
							<tr>
								...
								<td>
									...
									<xsl:value-of select="session"/>
									<xsl:text disable-output-escaping="yes">"&gt;</xsl:text><xsl:value-of select="substring-before(begin, 'T')"/> o <xsl:value-of select="substring-after(begin, 'T')"/><xsl:text disable-output-escaping="yes">&lt;/a&gt;</xsl:text>
								</td>
							</tr>
						</xsl:if>
					</table>
