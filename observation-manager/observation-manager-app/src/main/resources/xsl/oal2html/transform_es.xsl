		...
		<table border="0" cellspacing="0" cellpadding="2" style="font-size:12;font-family:Verdana, Arial">
			...
			<xsl:if test="count(apparentFOV)>0">
				...
				<tr>
					...
					<td>Campo aparente de visión:</td>
				...
				<xsl:if test="count(color)>0">
					...
					<tr>
						...
						<xsl:choose>
							...
							<xsl:when test="color='pale blue'">Azul pálido</xsl:when>
		...
		<ul>
			...
			<xsl:if test="contains(./@type,'findingsDeepSkyType') or contains(./@type,'findingsDeepSkyOCType') or contains(./@type,'findingsDeepSkyDSType')">
				...
				<li>Evaluación visual: 
						...
						<xsl:when test="contains(./@type,'findingsDeepSkyOCType')">
							...
							<xsl:choose>
								...
								<xsl:when test="rating = '1'">Muy evidente, cúmulo muy bueno</xsl:when>

								<xsl:when test="rating = '2'">Evidente, buen cúmulo</xsl:when>

								<xsl:when test="rating = '3'">Cúmulo visible con facilidad</xsl:when>

								<xsl:when test="rating = '4'">Cúmulo no muy visible</xsl:when>

								<xsl:when test="rating = '5'">Cúmulo visible con dificultad</xsl:when>

								<xsl:when test="rating = '6'">No es seguro haber divisado el cúmulo</xsl:when>

								<xsl:when test="rating = '7'">En la posición dada no se observan estrellas</xsl:when>
						...
						<xsl:otherwise>
							...
							<xsl:choose>
								...
								<xsl:when test="rating = '2'">Se ve claramente con visión directa</xsl:when>

								<xsl:when test="rating = '3'">Visible con visión directa</xsl:when>

								<xsl:when test="rating = '4'">Sólo es visible con visión lateral</xsl:when>

								<xsl:when test="rating = '5'">El objeto apenas puede ser visto con visión lateral</xsl:when>
				...
				<xsl:if test="count(smallDiameter)>0 and count(largeDiameter)>0">
					...
					<li>Tamaño aparente: <xsl:call-template name="angle">
	...
	<xsl:template match="/">
		...
		<HTML>
			...
			<HEAD>
				...
				<TITLE>Partes de observación</TITLE>

			</HEAD>

			<BODY>

				<div align="center" style="font-size:24;font-family:Verdana,Arial;color:#0000C0">Partes de observación</div>
