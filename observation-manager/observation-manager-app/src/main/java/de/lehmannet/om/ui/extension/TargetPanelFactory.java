...
public class TargetPanelFactory {
    public static AbstractPanel newInstance(
            IExtensionContext extensionContext, String xsiType, ITarget target, boolean editable) {
        switch (xsiType) {
            case GenericTarget.XML_XSI_TYPE_VALUE:
                return new GenericTargetPanel(
                        extensionContext.getConfiguration(), extensionContext.getModel(), target, editable);

            case TargetStar.XML_XSI_TYPE_VALUE:
                return new TargetStarPanel(
                        extensionContext.getConfiguration(), extensionContext.getModel(), target, editable);
            default:
