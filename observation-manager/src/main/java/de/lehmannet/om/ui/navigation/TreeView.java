...
public class TreeView extends JPanel implements TreeSelectionListener {
				...
				if( e.getButton() == MouseEvent.BUTTON3 ) {
			        ...
			        if(selRow != -1) {
			            if(e.getClickCount() == 1) {
			            	Object node = (Object)selPath.getLastPathComponent();
			            	
			         		if( node instanceof SchemaElementMutableTreeNode ) {
			         			ISchemaElement element = ((SchemaElementMutableTreeNode)node).getSchemaElement();
				            	new PopupMenuHandler(TreeView.this.observationManager, element, p.x, p.y, (byte)(PopupMenuHandler.EDIT + PopupMenuHandler.CREATE_HTML + PopupMenuHandler.CREATE_XML + PopupMenuHandler.DELETE + PopupMenuHandler.CREATE_NEW_OBSERVATION + PopupMenuHandler.EXTENSIONS), SchemaElementConstants.NONE, TreeView.this.observationManager.getExtensionLoader().getPopupMenus());        		
				            	...
				            	new PopupMenuHandler(TreeView.this.observationManager, null, p.x, p.y, PopupMenuHandler.CREATE, type, null);			         			
			         		}			            	
			            }
			        }					
				}				
    ...
    public void valueChanged(TreeSelectionEvent e) {
    	...
    	ISchemaElement element = null;    	
     	if(   ( node != null)
     	   && (!(node instanceof SchemaElementMutableTreeNode)) 
     	   ) {     	   
     		...
     		if( this.tree.isExpanded(ptp) ) {
     			...
     			/*if( this.lastSelected != null ) {
     				this.tree.setSelectionPath(new TreePath(this.lastSelected.getPath()));
     			}*/
     		} else {     			
     	...
     	if(   (node != null)
     	   && (node instanceof SchemaElementMutableTreeNode)
       		...
       		// If so, we pass the parent element to OM, which will force to load only the observations belonging to the parent element
       		// If not, observation will be null, which is force a load of all observations 
       		// Note: For all non IObservation elements, the updateRight() method argument parentElement is ignored. 
	...
	// Note: Passing the parent element, makes only sense if element is an IObservation
			...
			if( this.nodes.containsKey(parentElement) ) {
				...
				for(int i=0; i < current.getChildCount(); i++) {
					...
					if( element.equals(currentSE) ) {
						TreePath newPath = new TreePath(((SchemaElementMutableTreeNode)current.getChildAt(i)).getPath());
		...
		for(int x=0; x < elements.length; x++) {			
			...
			if( !(elements[x] instanceof IObservation) ) {
				...
				if( observations != null ) {
					// If the element is an IObserver, we also need to access the observations where this observer
					// is the coObserver
...
class SchemaElementMutableTreeNode extends DefaultMutableTreeNode {
	...
	public void setSize(int size, int secondValue) {
		...
		if( size >= 0 ) {
			if( secondValue > 0 ) {
				super.setUserObject("<html>" + element.getDisplayName() + " (" + size + " / <font color=\"#afafaf\">" + secondValue + "</font>)</html>");	
			} else {
...
class SchemaElementTreeCellRenderer extends DefaultTreeCellRenderer {
    ...
    public SchemaElementTreeCellRenderer(TreeView treeView,
        		...
        		if( schemaNode.getParent() instanceof SchemaElementMutableTreeNode ) {
        			...
        			if( parentSE instanceof IObserver ) {
        				if( parentSE.equals( ((IObservation)se).getObserver() ) ) {	// "Main" Observer
                			...
                			super.setForeground(new Color(175,175,175)); 			// Matches <font color=\"#afafaf\"> in MutableTreeNode
                			super.setFont(new Font("Arial",Font.ITALIC, 12));        			        					
        				}        				
