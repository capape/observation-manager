    	...
    	try {
        	while( iterator.hasNext() ) {
        		...
        		if( currentExtensionURL != null ) {    			
        			currentResult = this.checkForUpdates(currentExtension.getName(), currentExtension.getVersion(), currentExtensionURL);
        	...
        	// Check OM itself
        	currentResult = this.checkForUpdates("Observation Manager", FloatUtil.parseFloat(ObservationManager.VERSION), ObservationManager.UPDATE_URL);
    		if( currentResult != null ) { 	// New version found
    			result.add(currentResult);
    		}
	...
	private UpdateEntry checkForUpdates(String name, float oldVersion, URL checkURL) throws ConnectException {
		...
		try {
			...
			if( ioe instanceof UnknownHostException ) {
				...
				System.err.println("** Network connection not available or PROXY settings may be needed. Change the following line in obs.sh/obs.bat file to set proxy:");
				System.err.println("** start javaw -Djava.ext.dirs=.....  to");
				System.err.println("** start javaw -Dhttp.proxyHost={YOURPROXY} -Dhttp.proxyPort={YOURPROXYPORT} -Djava.ext.dirs=.....");
			} else {
