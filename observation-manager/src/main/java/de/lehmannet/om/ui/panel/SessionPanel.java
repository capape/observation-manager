...
public class SessionPanel extends AbstractPanel implements ActionListener, MouseListener {
  ...
  public SessionPanel(ObservationManager manager, ISession session, boolean editable) {
    ...
    this.language = new LanguageBox(this.observationManager.getConfiguration().getConfig(
        ObservationManager.CONFIG_CONTENTDEFAULTLANG), true);
    ...
    if (source instanceof JButton) {
      ...
      if (sourceButton.equals(this.selectCoObservers)) {
        ...
        if (this.coObsSelector.getAllSelectedElements() != null) {
          selected = new ArrayList(this.coObsSelector.getAllSelectedElements()); // Create new list instance, as
                                                                                 // otherwise the clear() below, will
                                                                                 // clear selection
        }
        ...
        if (this.beginDate != null) {
          ...
          dp = new DatePicker(this.observationManager, AbstractPanel.bundle.getString("panel.session.datePicker.start"));
        }
  ...
  public ISchemaElement updateSchemaElement() {
    ...
    this.session.setImages(this.imageContainer.getImages(this.observationManager.getXmlCache().getXMLFileForSchemaElement(session)));
  ...
  private void createPanel() {
    ...
    constraints.fill = GridBagConstraints.HORIZONTAL;
    OMLabel Llanguage = new OMLabel(AbstractPanel.bundle.getString("panel.session.label.language"), JLabel.RIGHT, false);
    ...
    constraints.fill = GridBagConstraints.HORIZONTAL;
    constraints.anchor = GridBagConstraints.WEST;
    OMLabel Lend = new OMLabel(AbstractPanel.bundle.getString("panel.session.label.end"), JLabel.LEFT, true);
     ...
     * Lsite.setToolTipText(AbstractPanel.bundle.getString("panel.session.tooltip.site")); gridbag.setConstraints(Lsite,
     * constraints); this.add(Lsite);
     */
    ...
    if ((this.session != null) // Show
      ...
      if (!(this.session.getCoObservers().isEmpty())) {
         ...
         * gridbag.setConstraints(LcoObservers, constraints); LcoObservers.setFont(new Font("sansserif",Font.ITALIC +
         * Font.BOLD, 12)); this.add(LcoObservers);
         */
        ConstraintsBuilder.buildConstraints(constraints, 0, ++yPos, 13, 1, 15, 45);
        this.coObserverTabbedPane = new JTabbedPane(JTabbedPane.TOP);
        this.coObserverTabbedPane.setBorder(BorderFactory.createTitledBorder(AbstractPanel.bundle
            .getString("panel.session.label.addObservers")));
        this.coObserverTabbedPane.setToolTipText(AbstractPanel.bundle.getString("panel.session.tooltip.addObservers"));
        JScrollPane scrollPane = new JScrollPane(this.coObserverTabbedPane, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
...
/*
      ...
      JLabel LimageContainer = new JLabel(AbstractPanel.bundle.getString("panel.observationItem.label.images"));
      ...
      JScrollPane imageContainerScroll = new JScrollPane(this.imageContainer, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
          JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
      imageContainerScroll.setBorder(BorderFactory.createTitledBorder(AbstractPanel.bundle
            .getString("panel.observationItem.label.images")));
