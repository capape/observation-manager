...
public class DeepSkyTargetMSPanel extends AbstractPanel implements ActionListener {
  ...
  private final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("de.lehmannet.om.ui.extension.deepSky.DeepSky", Locale.getDefault());	
    ...
    public DeepSkyTargetMSPanel(ObservationManager om, 
    		                    ITarget target, 
		...
		if(    (target != null)
		   && !(target instanceof DeepSkyTargetMS) 
		   ) {
			throw new IllegalArgumentException("Passed ITarget must derive from de.lehmannet.om.extension.deepSky.DeepSkyTargetMS\n");
		}
	...
	private void loadSchemaElement() {
		...
		if(    (targets != null)
		   && !(targets.isEmpty())
			...
			this.tableModel = new TargetTableModel((ITarget[])targets.toArray(new ITarget[] {}), this.observationManager);
	...
	private void createPanel() {	
		...
		/*DefaultTableCellRenderer renderer = (DefaultTableCellRenderer)this.componentStars.getDefaultRenderer(String.class);
	    ...
	    if( super.isEditable() ) {
		    ...
		    constraints.fill = GridBagConstraints.HORIZONTAL;
			JLabel Lnote = new JLabel(this.bundle.getString("panel.ms.note.doubleStar"), JLabel.CENTER);
