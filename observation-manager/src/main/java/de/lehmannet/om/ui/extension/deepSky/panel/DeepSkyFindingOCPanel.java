...
public class DeepSkyFindingOCPanel extends DeepSkyFindingPanel implements ActionListener {	
  ...
  public final String XSI_TYPE = "oal:findingsDeepSkyOCType";		
    ...
    public DeepSkyFindingOCPanel(ObservationManager om,
    						     IFinding finding, 
							     ISession s,
		  ...
		  if( !(finding instanceof DeepSkyFindingOC) )  {
			...
			try {  // Might be old (< 1.5) DeepSkyFinding
				if( finding instanceof DeepSkyFinding ) {
					super.finding = new DeepSkyFindingOC(finding.getDescription(), ((DeepSkyFinding)finding).getRating());	
				} else {  // Finding was something else. So recycle description and use 99 as default rating
					super.finding = new DeepSkyFindingOC(finding.getDescription(), 99);
				}
			} catch( ClassCastException cce ) {
				throw new IllegalArgumentException("Passed IFinding must derive from de.lehmannet.om.extension.deepSky.DeepSkyFindingOC\n");	
			}
		  } else {
		...
		this.imagePath = super.om.getInstallDir().getAbsolutePath() + File.separatorChar + "images" + File.separatorChar + "deepSky" + File.separatorChar + "clusterTypes" + File.separatorChar;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if( source instanceof JButton ) {
			if( source.equals(this.characterButton) ) {
				DeepSkyFindingOCTraitDialog traitDialog = new DeepSkyFindingOCTraitDialog(this.om, this.imagePath, this.character);
				this.character = traitDialog.getCharacter();
				if( this.character != null ) {
		        	Image image = Toolkit.getDefaultToolkit().getImage(this.imagePath + Character.toLowerCase(this.character.charValue()) + ".png");
		        	image = image.getScaledInstance(90,
                            						90,
													Image.SCALE_FAST);
		            this.characterButton.setIcon(new ImageIcon(image));
		            this.characterButton.setText("   " + this.character + "   ");
		            this.characterButton.setToolTipText(this.bundle.getString("dialog.oc.trait.explanation." + this.character.toString().toLowerCase()));
				} else {
	...
	public ISchemaElement createSchemaElement() {
		...
		super.finding = new DeepSkyFindingOC(super.findingContainer.getDescription(),
                                             rating);
	...
	private void createOCPanel() {
        ...
        if(   (this.finding != null)
           && (((DeepSkyFindingOC)this.finding).getCharacter() != null)
           ) {                  
        	Character c = ((DeepSkyFindingOC)this.finding).getCharacter();
        	Image image = Toolkit.getDefaultToolkit().getImage(this.imagePath + Character.toLowerCase(c.charValue()) + ".png");
        	image = image.getScaledInstance(90,
											90,
											Image.SCALE_FAST);
        	...
        	this.characterButton.setToolTipText(this.bundle.getString("dialog.oc.trait.explanation." + ((DeepSkyFindingOC)this.finding).getCharacter().toString().toLowerCase()));
        } else {
        	this.characterButton.setText(this.bundle.getString("panel.findingOC.button.character"));
