...
public class StatisticsDialog extends OMDialog implements ActionListener, ComponentListener {
	...
	public StatisticsDialog(ObservationManager om) {
		...
		if(   (observers == null) 
	       || (observers.length == 0)
			...
			String defaultObserverDisplayName = this.om.getConfiguration().getConfig(ObservationManager.CONFIG_DEFAULT_OBSERVER);
			...
			// Show popup
			SchemaElementSelectorPopup popup = new SchemaElementSelectorPopup(this.om, this.bundle.getString("dialog.statistics.observerPopup.title"), null, preselectedObserver, true, SchemaElementConstants.OBSERVER);
	...
	private void createCatalogueStatistics() {					
		...
		// Get config
		boolean useCoObservers = Boolean.valueOf(this.om.getConfiguration().getConfig(ObservationManager.CONFIG_STATISTICS_USE_COOBSERVERS)).booleanValue();
		...
		while( iterator.hasNext() ) {
			...
			checkers[i] = new CatalogChecker(current, observations, this.observers, useCoObservers, this.catProgress[i]);
...
class CatalogChecker implements Runnable {
	...
	public CatalogChecker(IListableCatalog catalog, 
	...
	public void run() {
		...
		// ---------- Checks (will add observation in catalogTargets object if target was found		
...
class UpdateRunnable implements Runnable {
	...
	public void run() {
		...
		synchronized( watchDog )  { 
			...
			while( 	 (counter <= this.dialog.checkers.length)
				  && (running)
				  ) {			// ...until all buttons are visible
