...
public class DeepSkyTargetOCPanel extends AbstractPanel {
  ...
  private final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("de.lehmannet.om.ui.extension.deepSky.DeepSky", Locale.getDefault());	
    ...
    public DeepSkyTargetOCPanel(ObservationManager om, 
    		                    ITarget target, 
		...
		if(    (target != null)
		   && !(target instanceof DeepSkyTargetOC) 
		   ) {
			throw new IllegalArgumentException("Passed ITarget must derive from de.lehmannet.om.extension.deepSky.DeepSkyTargetDN\n");
		}
    ...
    public ISchemaElement updateSchemaElement() {
		...
		if(   (clusterClassification != null)
		   && !("".equals(clusterClassification))
		   ) {
	...
	private void createPanel() {	
	    ...
	    OMLabel Lbrightest = new OMLabel(this.bundle.getString("panel.oc.label.brightestStar"), JLabel.RIGHT, false);
