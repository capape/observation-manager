...
public class SchemaUILoader {
	...
	public SchemaUILoader(ObservationManager om,
						  List extensions) {
	...
	public AbstractPanel getFindingPanel(String xsiType,
			                             IFinding finding,
										 ISession s,
										 boolean editable) {
		
		return this.getFindingPanelFromXSIType(xsiType,
											   finding, 
											   s,
											   null,
											   editable);
	...
	public AbstractPanel getFindingPanel(ITarget target,
							             IFinding finding,
										 boolean editable) {
		
		return this.getFindingPanel(target.getXSIType(),
									finding, 
									null,
									editable);
	...
	public ITargetDialog getTargetDialog(String xsiType,
										 ITarget target,
										 IObservation o) {
		
		return this.getTargetDailogFromXSIType(xsiType,
				                               target,
				                               o,
				                               true);
	...
	public AbstractPanel getTargetPanel(String xsiType,										
										ITarget target,
										IObservation o,
										boolean editable) {
		
		return this.getTargetPanelFromXSIType(xsiType,
											  target,
											  o,
											  editable);
	...
	public AbstractPanel getSchemaElementPanel(String xsiType,
											   int schemaElementConstant,
											   ISchemaElement schemaElement,
											   boolean editable) {
		
		return (AbstractPanel)this.getSchemaElementUIObject(xsiType, schemaElementConstant, schemaElement, editable, false);	
		
	}
	
	public AbstractDialog getSchemaElementDialog(String xsiType,
			   			 						 int schemaElementConstant,
			   									 ISchemaElement schemaElement,
			   									 boolean editable) {
		
		return (AbstractDialog)this.getSchemaElementUIObject(xsiType, schemaElementConstant, schemaElement, editable, true);
	...
	public String getTypeForDisplayName(String name) {
		...
		while( iterator.hasNext() ) {	// Iterator over all extensions
			...
			for(int i=0; i < types.length; i++) {	// Iterate over all types
				dispName = extension.getDisplayNameForXSIType(types[i]);	// Check if extension knows a displayname for this type								
				if(   (dispName != null)
				   && (name.equals(dispName))
				   ) {
	...
	private AbstractPanel getFindingPanelFromXSIType(String xsiType,
			                                         IFinding finding,
													 ISession s,
													 ITarget t,
			                                         boolean editable) {
		...
		return (AbstractPanel)this.loadByReflection(classname,
				                                    IFinding.class,
				                                    finding,
				                                    ISession.class,
													s,
	...
	private AbstractPanel getTargetPanelFromXSIType(String xsiType,
            									    ITarget target,
            									    IObservation o,
            										boolean editable) {
	...
	private ITargetDialog getTargetDailogFromXSIType(String xsiType,
            									     ITarget target,
            									     IObservation o,
            									     boolean editable) {
		...
		return (ITargetDialog)this.loadByReflection(classname,
													ITarget.class,
													target,
													IObservation.class,
													o,
    ...
    private Object loadByReflection(String classname,
    		                        Class exampleClass,
                       ...
                       && (parameters[1].isInstance(new Boolean(false)))
                            ...
                            break;        
                              ...
                              && (parameters[2].isInstance(new Boolean(false)))
                               ...
                               object = constructors[i].newInstance(new Object[] {this.observationManager, findingOrTarget, new Boolean(editable)});                           
                               break;
							...
							&& (parameters[3].isInstance(new Boolean(false)))
                             ...
                             object = constructors[i].newInstance(new Object[] {this.observationManager, findingOrTarget, additionalParameter1, new Boolean(editable)});                           
                             break;  
							...
							&& (parameters[4].isInstance(new Boolean(false)))
                             ...
                             object = constructors[i].newInstance(new Object[] {this.observationManager, findingOrTarget, additionalParameter1, additionalParameter2, new Boolean(editable)});                           
                             break;                               
    ...
    private Object getSchemaElementUIObject(String xsiType,
    										int schemaElementConstant,
    										ISchemaElement schemaElement,
    										boolean editable,
    										boolean dialog) {
    	...
    	return this.loadByReflection(classname, 
    							     exampleClass,
    								 schemaElement,
    								 null, 
    								 null, 
