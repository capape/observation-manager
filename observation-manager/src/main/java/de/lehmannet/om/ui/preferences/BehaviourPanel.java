...
public class BehaviourPanel extends PreferencesPanel {
  ...
  private PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("ObservationManager", Locale.getDefault());
	...
	public void writeConfig() {
		...
		super.configuration.setConfig(ObservationManager.CONFIG_IMAGESDIR_RELATIVE, "" + this.imagesPathRelative.isSelected());
        ...
        if(    ((currentValue == null || "".equals(currentValue.trim())) && defaultContentLanguage.getSelectedISOLanguage() != null) 
            || (defaultContentLanguage.getSelectedISOLanguage() != null && !defaultContentLanguage.getSelectedISOLanguage().equals(currentValue))
		   ) {
		...
		super.configuration.setConfig(ObservationManager.CONFIG_CONTENTDEFAULTLANG, this.defaultContentLanguage.getSelectedISOLanguage());
		   
        // ------------------
   
   		// Set default observer
   		if( this.defaultObserver.getSelectedSchemaElement() != null ) {
   			super.configuration.setConfig(ObservationManager.CONFIG_DEFAULT_OBSERVER, "" + this.defaultObserver.getSelectedSchemaElement().getDisplayName());
   		}
        ...
        // ------------------
   		...
   		if( this.catalogBox.getSelectedItem() != null ) {
   			super.configuration.setConfig(ObservationManager.CONFIG_DEFAULT_CATALOG, "" + this.catalogBox.getSelectedItem());
   		}
   		
        // ------------------
   		
   		// Set retrieve end date from session
   		super.configuration.setConfig(ObservationManager.CONFIG_RETRIEVE_ENDDATE_FROM_SESSION, "" + this.retrieveEndDateFromSession.isSelected());  		

        // ------------------
   		
   		// Set retrieve end date from session
   		super.configuration.setConfig(ObservationManager.CONFIG_STATISTICS_USE_COOBSERVERS, "" + this.useCoObserverInStatistics.isSelected());   		

        // ------------------
   		
   		// Set retrieve end date from session
   		super.configuration.setConfig(ObservationManager.CONFIG_CONSTELLATION_USEI18N, "" + !this.useLatinConstellationNames.isSelected());
	...
	private void createPanel() {
        ...
        this.imagesPathRelative.setSelected(Boolean.valueOf(super.configuration.getConfig(ObservationManager.CONFIG_IMAGESDIR_RELATIVE, "true")).booleanValue());
        ...
        this.retrieveEndDateFromSession.setSelected(Boolean.valueOf(super.configuration.getConfig(ObservationManager.CONFIG_RETRIEVE_ENDDATE_FROM_SESSION)).booleanValue());
        ...
        this.useCoObserverInStatistics.setSelected(Boolean.valueOf(super.configuration.getConfig(ObservationManager.CONFIG_STATISTICS_USE_COOBSERVERS, "true")).booleanValue());
        ...
        this.useLatinConstellationNames.setSelected(!Boolean.valueOf(super.configuration.getConfig(ObservationManager.CONFIG_CONSTELLATION_USEI18N, "true")).booleanValue());
	...
	private void fillCatalogBox() {
		...
		this.catalogBox.setSelectedItem(this.om.getConfiguration().getConfig(ObservationManager.CONFIG_DEFAULT_CATALOG));		
	...
	private boolean showSetLanguageDialog() {
	     ...
	     JOptionPane pane = new JOptionPane(this.bundle.getString("dialog.preferences.setLanguage2All.question"), JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_OPTION);
