...
public class StatisticsDetailsDialog extends AbstractDialog {
	...
	public StatisticsDetailsDialog(ObservationManager om,
			                       ObservationStatisticsTableModel model) {
	...
	private void exportObservedAsXML() {
    	...
    	new ProgressDialog(this.observationManager, 
	...
	private void exportObservedAsHTML() {
		...
		super.observationManager.createHTML(xmlHelper.getDocument(),
				                            getExportFile(catalogName + "_observed", "html"), 
				                            null);
	...
	private void exportMissingTargetAsHTML() {
    	...
    	// Call OM and let him to the second part of the export
		super.observationManager.createHTML(calculation.getDocument(), 
										    getExportFile(catalogName + "_missing", "html"), 
										    getXSLFile());
	...
	private File getXSLFile() {
		...
		String selectedTemplate = super.observationManager.getConfiguration().getConfig(ObservationManager.CONFIG_XSL_TEMPLATE);
		...
		File path = new File(super.observationManager.getInstallDir().getAbsolutePath()	+ File.separator + "xsl" + File.separator + selectedTemplate + File.separator + "targetOnly");
		if( !path.exists() ) {
			super.observationManager.createWarning(AbstractDialog.bundle.getString("warning.xslTemplate.dirDoesNotExist") + "\n" + path.getAbsolutePath());
			return null;
		}
		...
		if (!xslFile.exists()) {	// OK, maybe a language independent file can be found...			
			...
			if (!xslFile.exists()) {	// Nothing found, raise warning
				super.observationManager.createWarning(AbstractDialog.bundle.getString("warning.xslTemplate.noFileFoundWithName")
													   + "\n"
													   + path.getAbsolutePath()
													   + File.separator
													   + "targetsOnly"
...
/*    private Color currentBGColor = Color.WHITE;
    private HashMap rowColor = new HashMap();*/
	...
	public DetailPanel(ObservationManager om, AbstractSchemaTableModel model) {
        ...
        lsm.addListSelectionListener(new ListSelectionListener() {
                                                                    public void valueChanged(ListSelectionEvent e) {
                                                                        ...
                                                                        if (lsm.isSelectionEmpty()) {
                                                                           ...
                                                                           /* List l = (List)DetailPanel.this.model.getValueAt(selectedRow, 1);
                                                                            if(    (l != null)
                                                                               && !(l.isEmpty()) 
                                                                               ) {
        ...
        this.table.setDefaultRenderer(ITarget.class,
											...
											public Component getTableCellRendererComponent(JTable table,
												...
												if( value != null ) {					        		
									        		...
									        		/*if( DetailPanel.this.rowColor.containsKey(row) ) {
									        			cr.setBackground((Color)DetailPanel.this.rowColor.get(row));
									        		} else {
														if( DetailPanel.this.currentBGColor.equals(Color.WHITE) ) {
															DetailPanel.this.currentBGColor = Color.LIGHT_GRAY;
														} else {
        ...
        this.table.setDefaultRenderer(IObservation.class,
    		    new TableCellRenderer() {
    											public Component getTableCellRendererComponent(JTable table,
    																		                   Object value,
    																		                   boolean isSelected,
        ...
        /*
        this.table.setDefaultRenderer(List.class,
    		    new TableCellRenderer() {
    											public Component getTableCellRendererComponent(JTable table,
    																		                   Object value,
    																		                   boolean isSelected,
    												...
    												if( value != null ) {					        		
    						                        	...
    						                        	while( i.hasNext() ) {
    						                        		o = (IObservation)i.next();                        		
    						                    			df.setCalendar(o.getBegin());
    						                    			text = text + df.format(o.getBegin().getTime());                        		
    						                        		if( i.hasNext() ) {
    						                        			text = text + "; ";
    						                        		}
    						                        	}    									        		
