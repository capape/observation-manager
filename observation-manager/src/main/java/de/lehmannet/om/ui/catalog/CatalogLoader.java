...
public class CatalogLoader {
	...
	public CatalogLoader(ObservationManager om,
						 List extensions) {
	...
	public ITarget getTarget(String catalogName, 
			                 String objectID) {		
	...
	private void loadCatalogues() {
		...
		File catalogDir = new File(this.observationManager.getInstallDir().getAbsolutePath() + File.separator + CatalogLoader.CATALOG_DIR);
		...
		while( iterator.hasNext() ) {
			...
			if( this.knownExtensions.containsKey(current.getName()) ) {		// Is extension already known (we're in an update run)
			...
			// As catalogs are loaded during startup and loading of catalogs can take some time,
			// this should increase startup times
			CatalogLoaderRunnable runnable = new CatalogLoaderRunnable(current, 
					 												   this.catalogMap, 
					 												   catalogDir, 
...
class CatalogLoaderRunnable implements Runnable {
	...
	public CatalogLoaderRunnable(IExtension extension,
								 Map resultMap,
								 File catalogDir,
								 boolean debug) {
