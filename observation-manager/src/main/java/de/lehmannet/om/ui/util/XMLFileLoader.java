...
public class XMLFileLoader {
  ...
  public boolean saveAs(String oldPath, String newPath) {
    ...
    try {
      ...
      // this.loadObservations(newPath); // Fill cache .... Not good! Strange behaviour. After save, first try to do
      // chnaged (e.g. stellar etc) is not taken. Second try works...) Better solution below!
   ...
   * session.equals(observations[i].getSession()) ) { observationsFromSession.add(observations[i]); } } } else { return
   * null; // No observations at all }
   * 
  ...
  public boolean loadObservations(String xmlPath) {
    ...
    try {
      ...
      this.cache
          .add(new CacheEntry(xmlPath, obs, eye, fil, imager, observers, scopes, sessions, sites, targets, lenses));
...
class CacheEntry {
  ...
  // contained stars, so that we can remove the observation reference e.g. from TargetStar C
  ...
  public void addTarget(ITarget target) {
    ...
    if (!this.doublicateCheck(this.target, target)) {
      ...
      if (target instanceof ITargetContaining) {
        ...
        newContainingArray[newContainingArray.length - 1].addReferencedElements(((ITargetContaining) target)
  ...
  public void updateObservation(IObservation observation) {
    ...
    // After the remove, an updateObservation is called, which cannot find the observation any longer
    if (cacheEntry == null) {
      return;
    }
        ...
        for (int x = 0; x < this.target.length; x++) {
          if (this.target[x].getSchemaElement().equals(current)) { // Remove old cache reference
            ...
            if (this.target[x].getSchemaElement() instanceof ITargetContaining) {
              List containingTargets = ((ITargetContaining) this.target[x].getSchemaElement()).getComponentTargets(this
                  .getTargets());
    ...
    // E.g. an observation didn't have an filter, yet. But now it has one. This won't be covered
    // above as we only work on the cache entry (who doesn't know anything about the "new" element reference...
    // (Elements that existed before and are now removed from the observation, will be covered above!)
  ...
  public void updateSession(IObservation[] observations, ISession session) {
    ...
    for (int i = 0; i < this.coObserver.length; i++) { // Iterator over all coObservers
      ...
      while (oldReferedElementsIterator.hasNext()) {
        ...
        if (currentSE instanceof ISession) {
          if (session.getID().equals(currentSE.getID())) { // Is referenced session the searched session?
            if (!session.getCoObservers().contains(this.coObserver[i].getSchemaElement())) { // coObserver no longer
                                                                                             // part of session
      ...
      // Loop must take place here, as it could be that coObservers are added who didn't had any
      // references yet. (Observer that was never a coObserver before)
      if ((session.getCoObservers().contains(this.coObserver[i].getSchemaElement())) // We're in the current session's
                                                                                     // list
          && !(this.coObserver[i].getReferencedElements().contains(session)) // but we are not in the cached list of the
                                                                             // observer
      ) { // => we must be added
        // Add observations
  ...
  public void updateTarget(ITarget target) {
    ...
    for (int x = 0; x < this.targetContaining.length; x++) {
      if (this.targetContaining[x].getSchemaElement().equals(target)) {
        ...
        // CAUTION! FROM NOW ON this.targetContaining[] shall be no longer accessed in this method
        ...
        this.targetContaining[x].addReferencedElements(((ITargetContaining) target).getComponentTargets(this
            .getTargets()));
    ...
    while (newIterator.hasNext()) {
      ...
      if (!removedComponent.contains(current)) { // Old list didn't know this Target, so this must be new
        ...
        // OldComponents knows this Target, so remove it from the removedList (which is a copy of the oldList)
        // At the end of the while loop, this will leave only the removed Targets in the removedList
    ...
    for (int i = 0; i < this.eyepiece.length; i++) {
      if (this.eyepiece[i].getSchemaElement().equals(eyepiece)) {
        ...
        if ((foundElement) && (!foundDependency)) {
          ...
          if (i < newArray.length) { // Avoid ArrayIndexOutOfBoundsException if current element is the last and still
                                     // not found
      ...
      for (int i = 0; i < this.target.length; i++) {
        if (target.equals(this.target[i].getSchemaElement())) {
          ...
          if (this.target[i].getSchemaElement() instanceof ITargetContaining) {
            List containingTargets = ((ITargetContaining) this.target[i].getSchemaElement()).getComponentTargets(this
                .getTargets());
            ListIterator listIterator = containingTargets.listIterator();
            ITarget ct = null;
            while (listIterator.hasNext()) {
  ...
  public List removeTarget(ITarget target) {
    ...
    if ((foundElement) && (!foundDependency)) { // Object has no dependencies
      ...
      // As the ITargetContaining target must have no more observation references while deletion,
      // it's components must have lost its referenced observations as well. (handled in updateObservation or
      // deleteObservation)
