...
public class DatePicker extends JDialog {
	...
	private void initDialog() {
		...
		// Set table header
		String[] header = { this.bundle.getString("datePicker.sun"),
		...
		for(int x=0; x < header.length; x++) {
			headerPanel.add(new JLabel(header[x], JLabel.CENTER));	
		}
		...
		this.monthYearLabel = new JLabel("" + (this.month+1) + "/" + this.year, JLabel.CENTER);
