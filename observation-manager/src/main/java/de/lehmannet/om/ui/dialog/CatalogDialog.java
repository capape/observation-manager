...
class CatalogPanel extends AbstractPanel implements ActionListener {
                                                                        ...
                                                                        if (lsm.isSelectionEmpty()) {
                                                                			...
                                                                			for( int i=0; i < targets.length; i++ ) {
                                                                				if( targets[i].equals(CatalogPanel.this.selectedTarget) ) {                                                                					
                                                                					CatalogPanel.this.selectedTarget = targets[i];  // Return already existing target, instead of the newly created one
                                                                					break;
                                                                				}
                                                                			}                                                                            
    ...
    private void showSearchDialog() {
    	...
    	SearchDialog sd = new SearchDialog(AbstractDialog.bundle.getString("dialog.catalog.search.title"), panel, this, om);
		...
		if( this.selectedTarget != null ) {						
			...
			for( int i=0; i < targets.length; i++ ) {
				if( targets[i].equals(this.selectedTarget) ) {                                                                					
					this.selectedTarget = targets[i];  // Return already existing target, instead of the newly created one
					this.processComponentEvent(new ComponentEvent(this, ComponentEvent.COMPONENT_HIDDEN));
					return;
				}
			} 			
