...
public class TargetContainer extends Container implements MouseListener {
  ...
  final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("ObservationManager", Locale.getDefault());	
    ...
    public TargetContainer(ObservationManager om,
    	                   ITarget target, 
	       		           boolean editable,
    	...
    	this.constellationBox = new ConstellationBox(Boolean.valueOf(this.observationManager.getConfiguration().getConfig(ObservationManager.CONFIG_CONSTELLATION_USEI18N, "true")).booleanValue());
    ...
    public ITarget updateTarget() {    	
		...
		/*if( pos == null ) {
			this.createWarning(TargetContainer.bundle.getString("target.warning.posMalformed"));
			return null;				
		}*/
    ...
    private void loadSchemaElement() {
        ...
        if( c != null ) {
        	...
        	String cName = null;
			boolean i18N = Boolean.valueOf(this.observationManager.getConfiguration().getConfig(ObservationManager.CONFIG_CONSTELLATION_USEI18N, "true")).booleanValue();
			if( i18N ) {
        ...
        if(   (this.target.getDatasource() != null)
           && !("".equals(target.getDatasource()))
           ) {
    ...
    private void createPanel() {
        ...
        OMLabel LAliasName = new OMLabel(this.bundle.getString("target.label.aliasNames"), JLabel.RIGHT, false);
        ...
        if(   (this.target != null)   // Show
           && !(this.editable)
        	...
        	targetDatasourceLabel = new OMLabel(this.bundle.getString("target.label.datasource"), JLabel.RIGHT, true);
        	...
        	targetDatasourceLabel = new OMLabel(this.bundle.getString("target.label.datasourceObserver"), JLabel.RIGHT, true);
	...
	protected void createWarning(String message) {
		...
		JOptionPane.showMessageDialog(this,
				                      message,
				                      this.bundle.getString("target.warning.title"),
