...
public class ExtendedSchemaTableModel extends AbstractTableModel {
	...
	public ExtendedSchemaTableModel(ISchemaElement[] elements, int schemaElementType, String xsiFilter, boolean multipleSelection, List preSelectedTargets) {
		...
		for(int i=0; i < elements.length; i++) {
			...
			if(   (SchemaElementConstants.TARGET == schemaElementType)
			   && (xsiFilter != null)
			   ) {							
				if( !((IExtendableSchemaElement)elements[i]).getXSIType().equals(xsiFilter) ) {   // Check if xsiType matches
					continue;
				}
			}
			// In case of observations, filter xsiTypes of target
			if(   (SchemaElementConstants.OBSERVATION == schemaElementType)
			   && (xsiFilter != null)
			   ) {							
				if( !((IObservation)elements[i]).getTarget().getXSIType().equals(xsiFilter) ) {   // Check if xsiType matches
					continue;
				}
			}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		ISchemaElement keySchemaElement = (ISchemaElement)this.elementMap.keySet().toArray(new ISchemaElement[] {})[rowIndex];
    ...
    public void setSelection(int row, boolean selection) {
    	...
    	ISchemaElement keySchemaElement = (ISchemaElement)this.elementMap.keySet().toArray(new ISchemaElement[] {})[row];
