...
class CatalogTargets {
	...
	private boolean areEqual(ITarget catalogTarget, ITarget observedTarget) {   // Don't use equal() from ITarget, as it doesn't check aliasnames			

		// All names of the catalog target are now filled into an array. Mind that names are not formated!
		...
		if(   (catalogTargetAliasNames != null)
		   && (catalogTargetAliasNames.length > 0)
		   ) {
		...
		// All names of the observed target are now filled into an array. Mind that names are not formated!
		...
		if(   (observedTargetAliasNames != null)
		   && (observedTargetAliasNames.length > 0)
		   ) {
		...
		// Now compare both arrays and try to find a machting name. Start at the end of both arrays, as the target name is stored at the end.
		// Hopefully in most cases the names do already match.
		// Format name here should increase performance, as we only format the required amount of strings.
		for(int x = allCatalogTargetNames.length-1; x >= 0; x--) {
			for(int y = allObservedTargetNames.length-1; y >= 0; y--) {
				if( allCatalogTargetNames[x].equals(allObservedTargetNames[y]) ) {  // Try without formatting names (formatName call takes most of CPU time in whole calculation)
					return true;
				}
				if( this.formatName(allCatalogTargetNames[x]).equals(this.formatName(allObservedTargetNames[y])) ) {   // Try with formated names
					return true;	
				}					
			}
		}
