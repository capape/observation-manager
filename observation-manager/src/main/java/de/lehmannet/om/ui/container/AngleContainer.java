...
public class AngleContainer extends Container {
    ...
    public Angle getAngle() throws NumberFormatException  {
    	...
    	if( !this.useArc ) {
        	if(   (this.decValue.getText() != null)
	    	   && !("".equals(this.decValue.getText()))
	    	   ) {
	    		try {
    		...
    		if(   (this.arcDegreeValue.getText() != null)
    		   && !("".equals(this.arcDegreeValue.getText().trim()))
    		   ) {
    			int deg;
	    		try {
	    		...
	    		int min = 0;
	    		if(   (this.arcMinValue.getText() != null)
	    		   && !("".equals(this.arcMinValue.getText().trim()))
	    		   ) {
		    		try {
	    		...
	    		int sec = 0;
	    		if(   (this.arcSecValue.getText() != null)
	    		   && !("".equals(this.arcSecValue.getText().trim()))
	    		   ) {
		    		try {
	    		...
	    		EquPosition eq = new EquPosition("0" + EquPosition.RA_HOUR + "0" +  EquPosition.RA_MIN + "0" + EquPosition.RA_SEC, arcString);
    ...
    private void setArcValues() {
    	...
    	String min = arcString.substring(arcString.indexOf(EquPosition.DEC_DEG) + 1, arcString.indexOf(EquPosition.DEC_MIN));
    	String sec = arcString.substring(arcString.indexOf(EquPosition.DEC_MIN) + 1, arcString.indexOf(EquPosition.DEC_SEC));
