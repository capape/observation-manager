...
public class TableSorter extends AbstractSchemaTableModel {
	...
	public int modelIndex(int viewIndex) {
		...
		if(   (viewIndex == -1)
		   || (getViewToModel() == null)
		   || (getViewToModel().length == 0)
		   ) {
			return -1;
		}
		
		if(   (getViewToModel().length > viewIndex) 
		   && (getViewToModel()[viewIndex] != null)
		   ) {
	...
	private class SortableHeaderRenderer implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if (c instanceof JLabel) {
				...
				int modelColumn = 0;
				try {
					// Keep an eye on this: The next line seems to cause ArrayIndexOutofBounds sometimes 0>=0...don't know when it does so...					
