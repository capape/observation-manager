...
public class DeepSkyFindingPanel extends AbstractPanel implements IFindingPanel {	
	...
	protected final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("de.lehmannet.om.ui.extension.deepSky.DeepSky", Locale.getDefault());			
    ...
    public DeepSkyFindingPanel(ObservationManager om,
    						   IFinding finding, 
							   ISession s,
		...
		if(    (finding != null)
		   && !(finding instanceof DeepSkyFinding) ) {					
			//throw new IllegalArgumentException("Passed IFinding must derive from de.lehmannet.om.extension.deepSky.DeepSkyFinding\n");
			
			// Create DeepSkyFinding from given IFinding (must be a IFinding instance due to parameter definition)
	...
	public ISchemaElement createSchemaElement() {
		...
		this.finding = new DeepSkyFinding(this.findingContainer.getDescription(),
				                          rating);
	...
	void createPanel() {
        ...
        OMLabel LlargeDiameter = new OMLabel(this.bundle.getString("panel.finding.label.largeDiameter"), JLabel.RIGHT, false);
