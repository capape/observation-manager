...
public class SolarSystemTargetMoonPanel extends AbstractPanel {
  ...
  //private final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("de.lehmannet.om.ui.extension.solarSystem.SolarSystem", Locale.getDefault());	
    ...
    public SolarSystemTargetMoonPanel(ObservationManager om, 
                                      ITarget target, 
                                      IObservation o,
		...
		if(    (target != null)
	  	   && !(target instanceof SolarSystemTargetMoon) 
		) {
			throw new IllegalArgumentException("Passed ITarget must derive from de.lehmannet.om.extension.solarSystem.SolarSystemTargetMoon\n");
		}
	...
	private void createPanel() {	
        ...
        if(   (!super.isEditable())
        		...
        		if( site != null ) {
        			this.targetContainer.setPosition(Ephemerides.getMoonPosition(this.observation.getBegin(), site.getLongitude().toDegree()));	
        		}        		
        }           
