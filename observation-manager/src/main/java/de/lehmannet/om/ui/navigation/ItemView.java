...
public class ItemView extends JPanel implements ChangeListener {
    ...
    // Contains the current shown observation, or null (if other SchemaElement type is shown)
    ...
    public void showObservation(final IObservation observation) {
    	...
    	// Keep the last selected tab in mind...if there're more than one ( = we showed an observation)     	    	
    	...
    	if( f != null ) {
    		...
    		this.addFindingTab(f, observation.getSession(), observation.getTarget(), findingIndex);  // @todo: Only works with one finding	
    	}
    ...
    private void addObservationTab(IObservation observation) {
    	...
    	this.tabbedPane.addTab(this.bundle.getString("observation"),
    			               new ImageIcon(this.imagesDir + "observation_l.png"),
    			               this.overviewPanel);    	
    ...
    private void addFindingTab(IFinding finding, ISession session, ITarget t, int index) {
       	...
       	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		// 		Exception in thread "AWT-EventQueue-0" ERR  <some Date/Time>	java.lang.ArrayIndexOutOfBoundsException: 1
    		...
    		// After some investigation I found out that the showObservation() method in here has some hiccups
    		// when calling the addFindingTab method to add the finding as second tab (after the observationItem tab)
    		// (Position of this finding tab is 1...I think that correlated to the array index in the exception above)
    		// As a solution we use the invokeAndWait method now (only when adding this tab), and only when the calling
    		// thread is not the Swing EventDispatcher thread (which should only happen during startup, that the calling
    		// thread is not the EventDispatcher))
    		// As the invokeAndWait method cannot be called from the EventDispatcher thread, we use the if statement to
    		// decide whether we call it directly or via an own runnable.
    		// Also important to know is that we need to use invokeAndWait and NOT invokeLater method, as otherwise the
    		// sorted order of the tabs gets lost, and the finding tab is added somewhere at the end (instead of being the
    		// second tab
    		
    		if( SwingUtilities.isEventDispatchThread() ) {
    			this.tabbedPane.addTab(this.bundle.getString("finding"),
    								   new ImageIcon(this.imagesDir + "finding_l.png"),
		        	...
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
																		ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("finding"),
														               			                        new ImageIcon(ItemView.this.imagesDir + "finding_l.png"),
														                                                ItemView.FINDING_TIC); 
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
        		this.tabbedPane.addTab(this.bundle.getString("target"),
            			   new ImageIcon(this.imagesDir + "target_l.png"),
            			   ItemView.TARGET_TIC);  
    		} else {
    			try {
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
															    		ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("target"),
														               			   new ImageIcon(ItemView.this.imagesDir + "target_l.png"),
														               			   ItemView.TARGET_TIC);  
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
    	    	this.tabbedPane.addTab(this.bundle.getString("site"),
			               new ImageIcon(this.imagesDir + "site_l.png"),
			               ItemView.SITE_TIC);
    		} else {
    			try {
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
																    	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("site"),
													    			               new ImageIcon(ItemView.this.imagesDir + "site_l.png"),
													    			               ItemView.SITE_TIC);
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
    	    	this.tabbedPane.addTab(this.bundle.getString("session"),
			               new ImageIcon(this.imagesDir + "session_l.png"),
		        	...
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
																    	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("session"),
																	               new ImageIcon(ItemView.this.imagesDir + "session_l.png"),
																	               ItemView.SESSION_TIC);
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
    	    	this.tabbedPane.addTab(this.bundle.getString("scope"),
            			   new ImageIcon(this.imagesDir + "scope_l.png"),
            			   ItemView.SCOPE_TIC);
    		} else {
    			try {
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
																    	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("scope"),
														               			   new ImageIcon(ItemView.this.imagesDir + "scope_l.png"),
														               			   ItemView.SCOPE_TIC);
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
            	this.tabbedPane.addTab(this.bundle.getString("observer"),
            			   new ImageIcon(this.imagesDir + "observer_l.png"),
		        	...
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
															        	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("observer"),
														               			   new ImageIcon(ItemView.this.imagesDir + "observer_l.png"),
														               			   ItemView.OBSERVER_TIC);
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
    	    	this.tabbedPane.addTab(this.bundle.getString("eyepiece"),
			               new ImageIcon(this.imagesDir + "eyepiece_l.png"),
		        	...
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
																    	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("eyepiece"),
													    			               new ImageIcon(ItemView.this.imagesDir + "eyepiece_l.png"),
													    			               ItemView.EYEPIECE_TIC);
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
            	this.tabbedPane.addTab(this.bundle.getString("filter"),
			               new ImageIcon(this.imagesDir + "filter_l.png"),
			               ItemView.FILTER_TIC);
    		} else {
    			try {
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
															        	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("filter"),
																	               new ImageIcon(ItemView.this.imagesDir + "filter_l.png"),
																	               ItemView.FILTER_TIC);
																	}
															    } 
												 );
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
            	this.tabbedPane.addTab(this.bundle.getString("imager"),
			               new ImageIcon(this.imagesDir + "imager_l.png"),
			               ItemView.IMAGER_TIC); 
    		} else {
    			try {
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
															        	ItemView.this.tabbedPane.addTab(ItemView.this.bundle.getString("imager"),
																	               new ImageIcon(ItemView.this.imagesDir + "imager_l.png"),
																	               ItemView.IMAGER_TIC); 
																	}
															    } 
												 );
    ...
    private void loadImagerPanel(IImager imager, int index) {
    	...
    	this.imagerPanel = this.main.getExtensionLoader().getSchemaUILoader().getSchemaElementPanel(imager.getXSIType(), SchemaElementConstants.IMAGER, imager, false);
    	...
    	if( this.tabbedPane.getTabCount() == 0 ) { // Show only this tab
    		...
    		if( SwingUtilities.isEventDispatchThread() ) {
            	this.tabbedPane.addTab(tabTitle,
			               new ImageIcon(this.imagesDir + "lens_l.png"),
			               ItemView.LENS_TIC);
    		} else {
    			try {
		        	SwingUtilities.invokeAndWait(new Runnable() {
																	public void run() {		
															        	...
															        	ItemView.this.tabbedPane.addTab(tabTitle,
												        			               new ImageIcon(ItemView.this.imagesDir + "lens_l.png"),
												        			               ItemView.LENS_TIC);
																	}
															    } 
												 );
    ...
    private void loadLensPanel(ILens lens, int index) {
    	...
    	this.lensPanel = new LensPanel((Lens)lens, false);
