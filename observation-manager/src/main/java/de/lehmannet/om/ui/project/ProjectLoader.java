...
public class ProjectLoader {
	...
	private void loadProjects() {
		...
		File path = new File(this.observationManager.getInstallDir().getAbsolutePath() + File.separator + ProjectLoader.PROJECTS_DIR);
		if( !path.exists() ) {			
			return;
		}	
		...
		// As projects are loaded during startup and loading of projects can take some time,
		// this should increase startup times
		// It must be ensured that catalogs are loaded completely before the projects, as projects
		// refer to catalogs
		ArrayList projectThreads = new ArrayList();
		File projectFile = null; 
		for(int i=0; i < projects.length; i++) {
			projectFile = new File(path.getAbsolutePath() + File.separator + projects[i]);
			ProjectLoaderRunnable runnable = new ProjectLoaderRunnable(this.observationManager,
																	   this.projectList, 
																	   userTargets,
				 												       projectFile, 
...
class ProjectLoaderRunnable implements Runnable {
	...
	public ProjectLoaderRunnable(ObservationManager om,
								 List projectList,
								 List userTargets,
								 File projectFile,
								 boolean debug) {
	...
	private ITarget searchForTarget(String line) {
		...
		if(   (catalogName != null)
		   && (USER_KEY.equals(catalogName.toUpperCase()))
		   ) {
			String ut_name = null;
			...
			if( !this.userTargets.isEmpty() ) {	// There are userTargets at all
				...
				while( iterator.hasNext() ) {
					...
					if( ut_name.equals(t_name) ) {	// Target names match
						...
						if(   (current.getAliasNames() != null)
						   && (current.getAliasNames().length > 0)
						   ) {
	...
	public ProjectCatalog loadProjectCatalog(File projectFile) { 
		...
		try {
			...
			while( (line = reader.readLine()) != null ) {
				...
				if( target == null ) {
					// Throw exception as target does not exist (neither in catalogs, nor in XML file)
...
class WaitPopup extends OMDialog {
    ...
    public WaitPopup(ThreadGroup threadGroup,
