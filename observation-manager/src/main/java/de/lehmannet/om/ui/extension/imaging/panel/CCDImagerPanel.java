...
public class CCDImagerPanel extends AbstractPanel implements MouseListener, IImagerPanel {
  ...
  private final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("de.lehmannet.om.ui.extension.imaging.Imaging", Locale.getDefault());
    ...
    public CCDImagerPanel(IImager imager, 
    					  Boolean editable) {
		...
		if( imager != null ) {
			if( !(imager instanceof CCDImager) ) {
				throw new IllegalArgumentException("Passed Imager must be of type: de.lehmannet.om.extension.imaging.CCDImager");
			}
	...
	public ISchemaElement createSchemaElement() {
		...
		// Create ccd imager
		this.imager = new CCDImager(modelName, 
				                    x,
				                    y);
