...
public class GCVS4SearchPanel extends AbstractSearchPanel {
	...
	public GCVS4SearchPanel(GCVS4Catalog catalog) {
		...
		// Set cached values
		if( Boolean.valueOf(catalog.observationManager.getConfiguration().getConfig(VariableStarsPreferences.CONFIG_CACHE_ENABLED)).booleanValue() ) {
	...
	public void search(String searchString) {
		...
		if( Boolean.valueOf(catalog.observationManager.getConfiguration().getConfig(VariableStarsPreferences.CONFIG_CACHE_ENABLED)).booleanValue() ) {
			catalog.observationManager.getConfiguration().setConfig(GCVS4SearchPanel.CONFIG_LAST_SEARCHTERM, searchString);
		}
