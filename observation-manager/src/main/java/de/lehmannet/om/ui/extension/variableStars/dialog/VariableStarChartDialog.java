...
public class VariableStarChartDialog extends OMDialog implements PropertyChangeListener, ComponentListener {
	...
	public VariableStarChartDialog(ObservationManager om,
								   IObservation[] observations,
								   Map colorMap) {
	
		super(om);
		
		super.setTitle(this.bundle.getString("chart.title") + " " + observations[0].getTarget().getName());
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		// As we only listen to the exit property...no need to check anything in here. Just close the dialog
		super.dispose();
...
class MagnitudeDiagramm extends JPanel implements MouseListener {
	...
	private TreeSet observations = null;
	...
	// Start value of axis (y-Axis starts with highest number in origin, as magnitude value increases with lesser brightness
	private float minXValue = 0.0f;
	private float maxYValue = 0.0f;
	
	// Transform values for x and y (Offset for origin, caused by borders, titles, etc.)
	...
	// In case the axis values delta (max-min) is too huge, use divisor to reduce segment number
	...
	public MagnitudeDiagramm(ObservationManager om,
			                 TreeSet observations,
			                 Map colorMap) {
	...
	public void mousePressed(MouseEvent e) {
		...
		this.xPopup = new Integer(Math.round(x));
		this.yPopup = new Integer(Math.round(y));
	...
	private void paintTitle() {
		...
		if(   !(Float.isNaN(variableStar.getMagnitudeApparent())) 
		   && !(Float.isNaN(variableStar.getMaxApparentMag())) 
		   ) {
		...
		String fromDateJD = "JD: " + DateConverter.toJulianDate(((IObservation)(this.observations.first())).getBegin());		
		...
		int maxFromDateLength = valueFontMetric.stringWidth(fromDateJD) >= valueFontMetric.stringWidth(fromDate) ? valueFontMetric.stringWidth(fromDateJD) : valueFontMetric.stringWidth(fromDate);		
		...
		this.g2d.drawString(toDateLabel, valueWidth + 15 + valueWidthFromLabel + 10 + maxFromDateLength + 20, valueHeightFromLabel);
		
		g2d.setFont(valueFont);		
		this.g2d.drawString(toDateJD, valueWidth + 15 + valueWidthFromLabel + 10 + maxFromDateLength + 20 + valueWidthToLabel + 10, valueHeightFromLabel);		
		this.g2d.drawString(toDate, valueWidth + 15 + valueWidthFromLabel + 10 + maxFromDateLength + 20 + valueWidthToLabel + 10, valueHeightFromLabel * 2);
		...
		this.g2d.drawString(noOfObservationsLabel, valueWidth + 15 + valueWidthFromLabel + 10 + maxFromDateLength + 20 + valueWidthToLabel + 10 + maxFromDateLength + 20, valueHeightFromLabel);
				
		g2d.setFont(valueFont);
		String noOfObservation = "" + this.observations.size();		
		this.g2d.drawString(noOfObservation, valueWidth + 15 + valueWidthFromLabel + 10 + maxFromDateLength + 20 + valueWidthToLabel + 10 + maxFromDateLength + 20 + valueWidthNoOfObservation + 10, valueHeightFromLabel);
		...
		int counter = 1;
		int xValuePos = labelFontMetric.stringWidth(originLabel) > labelFontMetric.stringWidth(magFromToLabel) ? labelFontMetric.stringWidth(originLabel) : labelFontMetric.stringWidth(magFromToLabel);
		xValuePos = xValuePos > labelFontMetric.stringWidth(periodLabel) ? xValuePos : labelFontMetric.stringWidth(periodLabel);
		xValuePos = xValuePos > labelFontMetric.stringWidth(typeLabel) ? xValuePos : labelFontMetric.stringWidth(typeLabel);
	...
	private void paintPopup() {
		...
		for(int i=0; i < this.dataSpots.length; i++) {
			if( this.dataSpots[i].contains(this.xPopup.intValue(), this.yPopup.intValue()) ) {	// If click was inside spot, paint popup				
				...
				// The popup text
				SimpleDateFormat format = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss", Locale.getDefault());				
	        	format.setCalendar(this.dataSpots[i].getObservation().getBegin());
	        	
	        	String sign = this.dataSpots[i].getResult().isMagnitudeFainterThan() ? "<" : "";
				String info[] = new String[] {(sign + this.dataSpots[i].getResult().getMagnitude() + "mag"),  
								              ("JD: " + DateConverter.toJulianDate(this.dataSpots[i].getObservation().getBegin())),
							                  (this.bundle.getString("chart.popup.date") + ": " + format.format(this.dataSpots[i].getObservation().getBegin().getTime()))
											  };
				
				// Get largest popup text
				String maxInfoString = "";
				for(int j=0; j < info.length; j++) {
					maxInfoString = maxInfoString.length() < info[j].length() ? info[j] : maxInfoString;
				}
				...
				if( (Math.round(this.xPopup.intValue() + this.dataSpots[i].width * 2) + width + 4) >= super.getWidth() ) {   // Popup doesn't fit into frame
					// Paint popup
					this.g2d.drawRect(Math.round(this.xPopup.intValue() - (this.dataSpots[i].width * 2) - width), this.yPopup.intValue() + 2, width, valueHeight * info.length + (valueHeight / 4 * info.length));
					
					// Paint value into popup
					for(int j=0; j < info.length; j++) {
						this.g2d.drawString(info[j], this.xPopup.intValue() - (this.dataSpots[i].width * 2) - width + 4, this.yPopup.intValue() + ((j+1) * valueHeight) + (valueHeight / 4 * j));	
					}						
				} else {
					// Paint popup
					this.g2d.drawRect(Math.round(this.xPopup.intValue() + this.dataSpots[i].width * 2), this.yPopup.intValue() + 2, width, valueHeight * info.length + (valueHeight / 4 * info.length));
					
					// Paint value into popup
					for(int j=0; j < info.length; j++) {
						this.g2d.drawString(info[j], this.xPopup.intValue() + this.dataSpots[i].width * 2 + 4, this.yPopup.intValue() + ((j+1) * valueHeight) + (valueHeight / 4 * j));	
					}						
				}
	...
	private void paintData() {		
		...
		// We need to create dataSpots again with each repaint, as canvas/frame size might change
		...
		while( iterator.hasNext() ) {
			...
			currentValueXPos = (float)(((DateConverter.toJulianDate(current.getBegin()) - this.minXValue) / this.xDivisor) * this.xAxisSegmentSize) - (CIRCLE_DIAMETER / 2);
			currentValueYPos = (((this.maxYValue - ((FindingVariableStar)(current.getResults().get(0))).getMagnitude()) / this.yDivisor) * -this.yAxisSegmentSize * 10) - (CIRCLE_DIAMETER / 2);				
			
			this.dataSpots[i] = new DataSpot(currentValueXPos, currentValueYPos, CIRCLE_DIAMETER, CIRCLE_DIAMETER, current);
	...
	private void paintColorCode() {
		...
		Iterator iterator = this.colorMap.keySet().iterator();
		while( iterator.hasNext() ) {		
			currentObserver = (IObserver)iterator.next();
			...
			if(   (currentObserver.getUsernameForAccount(Observer.ACCOUNT_AAVSO) != null)
			   && !("".equals(currentObserver.getUsernameForAccount(Observer.ACCOUNT_AAVSO).trim()))		
				...
				currentObserverLabel = currentObserverLabel + " (" + currentObserver.getUsernameForAccount(Observer.ACCOUNT_AAVSO) + ")";
			}			
			
			// Check if this line should start (make a forecast here)
			if( (currentX + CIRCLE_DIAMETER + 15 + labelFontMetric.stringWidth(currentObserverLabel)) >= super.getSize().getWidth() - BORDER_RIGHT ) {
				rowNo++;
			...
			Shape spot = new Ellipse2D.Float(currentX,
											 (int)(super.getSize().getHeight() - (BORDER_BOTTOM / 1.4)+ (rowGap * rowNo)),
											 CIRCLE_DIAMETER,
											 CIRCLE_DIAMETER);
			g2d.draw(spot);
			g2d.fill(spot);
			...
			// Paint observer name
			this.g2d.drawString(currentObserverLabel, currentX, (int)(super.getSize().getHeight() - (BORDER_BOTTOM / 1.4) + (rowGap * rowNo) + (labelFontMetric.getHeight() / 2)));
	...
	private void paintAxis() {
		...
		this.yTransformValue = (int)Math.round(height-BORDER_BOTTOM);   // Use BORDER_BOTTOM as y axis counts ascending downwards
		...
		this.xAxisSegmentSize = xAxisLength / xAxisSegmentNumber;
		...
		for(int x=1; x <= xAxisSegmentNumber; x++) {
			g2d.drawLine(Math.round(xAxisSegmentSize * x), 0 - (lengthOfSegmentLines / 2), Math.round(xAxisSegmentSize * x), 0 + (lengthOfSegmentLines / 2));
		}
		...
		while(   ((labelWidth +10) > (xAxisSegmentSize * skipLabel))  // Does label (+5 pixels space on each side) not fit into segment?
		...
		for(int x=0; x <= xAxisSegmentNumber; x++) {
			if( (((float)x / skipLabel) % 1) == 0.0f ) {	 // Make sure we draw only the labels that fit (respective to their width) on the axis
				g2d.drawString(formatX.format(currentLabel/10.0), (xAxisSegmentSize * x) - labelWidth / 2, 0 + labelHeight + lengthOfSegmentLines);	
			}
		...
		g2d.drawString(xAxisLabel, (xAxisLength / 2) - (axisLabelWidth / 2), 0 + labelHeight + lengthOfSegmentLines + axisLabelHeight);
		...
		int yAxisSegmentNumber = Math.round((maxYValue - minYValue)*10) > 0 ? Math.round((maxYValue - minYValue) * 10) : 10;  // draw in 0.1 mag steps
		if( yAxisSegmentNumber > 20 ) { // Too many segments
			this.yDivisor = (float)(yAxisSegmentNumber / 20.0);
			yAxisSegmentNumber = 20;
		}
		this.yAxisSegmentSize = yAxisLength / yAxisSegmentNumber;		
		
		// Draw y axis segments
		for(int x=1; x <= yAxisSegmentNumber; x++) {
			g2d.drawLine(Math.round(0 - (lengthOfSegmentLines / 2)), Math.round(-yAxisSegmentSize * x), Math.round(0 + (lengthOfSegmentLines / 2)), Math.round(-yAxisSegmentSize * x));
		}	
		...
		while(   ((labelHeight +10) > (yAxisSegmentSize * skipYLabel))  // Does label (+5 pixels space on top and bottom) not fit into segment?
		...
		// Draw labels
		// The magic in here is the currentYLabel incremental which doesn't work exactly if we add 0.1 in each step, due to javas unprecise
		...
		for(int x=0; x <= yAxisSegmentNumber; x++) {
			if( (((float)x / skipYLabel) % 1) == 0.0f ) {	 // Make sure we draw only the labels that fit (respective to their height) on the axis
				g2d.drawString(formatY.format(currentYLabel/10), 0 - Math.round(labelWidth * 1.3), (-yAxisSegmentSize * x) + (labelHeight / 2));	
			}
		...
		for(int i=0; i < yAxisLabel.length(); i++) {
			g2d.drawString("" + yAxisLabel.charAt(i), (int)(0 - (labelWidth * 1.7) - yAxisLabelWidth), ((-yAxisLength / 2) - (yAxisLabelHeight / 2)) + i * (yAxisLabelCharHeight + 5));
		}		
	...
	public void saveImage() {
		...
		BufferedImage awtImage = new BufferedImage(super.getWidth(),
												   super.getHeight(),
	...
	private TreeSet getObservations(TreeSet observations, Map colorMap) {
		
		Iterator iterator = observations.iterator();
		IObservation current = null;
		while( iterator.hasNext() ) {
			current = (IObservation)iterator.next();
			if( colorMap.get(current.getObserver()) == null ) {		// No color assigned...so remove observation
				iterator.remove();
			}
		}
...
class ChartPopupHandler implements ActionListener {
	...
	public ChartPopupHandler(int x, int y, MagnitudeDiagramm chart) {
		...
		this.save = new JMenuItem(this.bundle.getString("chart.popup.save"));
		this.save.addActionListener(this);
		popupMenu.add(this.save);

		this.exit = new JMenuItem(this.bundle.getString("chart.popup.exit"));
