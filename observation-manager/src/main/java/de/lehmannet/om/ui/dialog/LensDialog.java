...
public class LensDialog extends AbstractDialog {
	...
	public LensDialog(ObservationManager om,
			          ILens editableLens) {
		...
		if( editableLens == null ) {
			...
			if( editableLens.getFactor() >= 1 ) {
				super.setTitle(AbstractDialog.bundle.getString("dialog.lens.barlow.titleEdit") + " " + editableLens.getDisplayName());	
			} else {
				super.setTitle(AbstractDialog.bundle.getString("dialog.lens.shapley.titleEdit") + " " + editableLens.getDisplayName());
			}			
		}			
