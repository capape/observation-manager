...
public class HorizontalSkymap extends JLabel {
	...
	private void createImage() {
		...
		BufferedImage image = new BufferedImage(HorizontalSkymap.WIDTH, HorizontalSkymap.HEIGHT, BufferedImage.TYPE_INT_ARGB);
	...
	private void paintSky(BufferedImage image) {
		...
		g2d.fillOval(HorizontalSkymap.SKY_BORDER, HorizontalSkymap.SKY_BORDER, (HorizontalSkymap.WIDTH) - 2 * HorizontalSkymap.SKY_BORDER, (HorizontalSkymap.HEIGHT) - 2 * HorizontalSkymap.SKY_BORDER);
	...
	private void paintObjectPosition(BufferedImage image) {
		...
		// Get object location
		double azimut = Ephemerides.getAzimut(this.position, this.calendar, this.site.getLongitude().getValue(), this.site.getLatitude().getValue());
		double altitude = Ephemerides.altitudeAboveHorizon(this.position, this.calendar, this.site.getLongitude().getValue(), this.site.getLatitude().getValue());
		...
		Point center = new Point(HorizontalSkymap.WIDTH / 2, HorizontalSkymap.HEIGHT / 2);	// Center point of sky (zenith)
		...
		// Substract radius at the end to make sure 90 deg is on the center of the circle (radius =0)
		double altitudeOnRadius = Math.abs(radius - (radius / (90 / altitude))); 
		
		// Claculate location of object point (90 - azimut as 'north' is 0 degrees for us and not 'east' or x-axis)
		...
		// Caution! The skymap is  astronomically oriented! East is 'left' and azimut 0 is north or 'top'
		if(   (azimut > 0)
		   && (azimut < 90)
		   ) {
			if( x > 0 ) {
	...
	private void paintLabel(BufferedImage image) {
		...
		g2d.drawChars(west.toCharArray(), 0, west.length(), HorizontalSkymap.WIDTH - 9, HorizontalSkymap.HEIGHT / 2 + 3);
