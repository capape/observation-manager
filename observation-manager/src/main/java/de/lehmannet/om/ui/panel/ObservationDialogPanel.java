...
public class ObservationDialogPanel extends AbstractPanel implements ActionListener, ItemListener, ChangeListener {
  ...
  private JTextField end = null;
	...
	// Receives (non-persistent) cache in order to preset some UI values with recent values
	public ObservationDialogPanel(ObservationManager om,
    ...
    public ISchemaElement updateSchemaElement() {
		...
		if( site != null ) {
			...
			tz = SimpleTimeZone.getDefault();	// Use hosts default timezone
		}
		...
		// Setting of end date must be before setting of session, as otherwise check in setting session (start-end-date) will fail
		...
		try {
			...
			if(   (session != null) 
			   ...
			   && (session.getCoObservers().contains(observer))
				 ...
				 JOptionPane pane = new JOptionPane(AbstractPanel.bundle.getString("panel.observation.warning.coObserverIsObserver"),
			...
			this.cache.put(ObservationDialogPanel.CACHEKEY_SESSION, (ISession)this.sessionBox.getSelectedSchemaElement());
		...
		this.cache.put(ObservationDialogPanel.CACHEKEY_SCOPE, (IScope)this.scopeBox.getSelectedSchemaElement());
...
/*		
		if(   (s != null)
		   && !("".equals(s.trim()))
			...
			try {
				int seeing = Integer.parseInt(s);
				try {
					this.observation.setSeeing(seeing);	
				} catch(IllegalArgumentException iae) {
					this.createWarning(AbstractPanel.bundle.getString("panel.observation.warning.invalidSeeing"));
					return null;
				}		
			} catch(NumberFormatException nfe) {
				this.createWarning(AbstractPanel.bundle.getString("panel.observation.warning.noNumberSeeing"));
				return null;
			}
		} else {
			this.observation.setSeeing(-1);
		...
		this.observation.setImages(this.imageContainer.getImages(this.observationManager.getXmlCache().getXMLFileForSchemaElement(this.observation)));		
	...
	public ISchemaElement createSchemaElement() {
		...
		this.observation = new Observation(this.beginDate,
				                           target,
										   observer,
										   finding);
		...
		try {
			...
			if( se != null ) {
				...
				this.cache.put(ObservationDialogPanel.CACHEKEY_SESSION, (ISession)se);
			}			
		} catch(IllegalArgumentException iae) {
			super.createWarning(AbstractPanel.bundle.getString("panel.observation.warning.wrongTimeForSession"));
			return null;
		...
		if( se != null ) {
			...
			this.cache.put(ObservationDialogPanel.CACHEKEY_SCOPE, (IScope)se);
		}		
		...
		if( se != null ) {
			...
			this.cache.put(ObservationDialogPanel.CACHEKEY_SITE, (ISite)se);
		}		
	...
	public void actionPerformed(ActionEvent e) {
		...
		if( source instanceof JButton ) {
			if( source.equals(this.newEyepiece) ) {
				...
				ExtenableSchemaElementSelector imagerChooser = new ExtenableSchemaElementSelector(this.observationManager, 
				...
				ExtenableSchemaElementSelector targetChooser = new ExtenableSchemaElementSelector(this.observationManager, 
						                                          								  this.observationManager.getExtensionLoader().getSchemaUILoader(),
				...
				if( this.beginDate != null ) {					
					dp = new DatePicker(this.observationManager, AbstractPanel.bundle.getString("panel.observation.start.datePicker.title"), this.beginDate, timeZone);	
				} else {
					dp = new DatePicker(this.observationManager, AbstractPanel.bundle.getString("panel.observation.start.datePicker.title"), timeZone);
				}				
				this.beginDate = dp.getDate();
				this.beginTime.setTime(this.beginDate.get(Calendar.HOUR_OF_DAY), 
									   this.beginDate.get(Calendar.MINUTE),
				...
				if( this.endDate != null ) {
					dp = new DatePicker(this.observationManager, AbstractPanel.bundle.getString("panel.observation.end.datePicker.title"), this.endDate, timeZone);
				} else if(this.beginDate != null) {  // Try to initialize endDate Picker with startdate					
					dp = new DatePicker(this.observationManager, AbstractPanel.bundle.getString("panel.observation.end.datePicker.title"), this.beginDate, timeZone);
				} else {
					dp = new DatePicker(this.observationManager, AbstractPanel.bundle.getString("panel.observation.end.datePicker.title"), timeZone);
				}
				this.endDate = dp.getDate();
				this.endTime.setTime(this.endDate.get(Calendar.HOUR_OF_DAY), 
        ...
        Calendar now = Calendar.getInstance();
        this.beginDate = now;
        this.beginTime.setTime(now.get(Calendar.HOUR_OF_DAY), now.get(Calendar.MINUTE), now.get(Calendar.SECOND));
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if( source.equals(this.targetBox) ) {
	        ...
	        if(   (e.getStateChange() == ItemEvent.SELECTED)  
	          // && (this.observation == null)	// (Autom. set date only in Creation mode. In edit mode this would set session dates as begin and end)
               ) {
	        	if (session != null) {
	        		...
	        		if( observation != null ) {
		        		if( session.equals(observation.getSession()) ) {
		        			// As we unset the site below (in else case) we have to make sure it's set here again
		        			// Test: Session and Site set. Switch Session to "----", -> Site will also be set to "----"
			        ...
			        this.beginTime.setTime(start.get(Calendar.HOUR_OF_DAY),
			        		               start.get(Calendar.MINUTE),
			        		               start.get(Calendar.SECOND));

			        // Check whether enddate/time should be autom. set
			        if( Boolean.valueOf(this.observationManager.getConfiguration().getConfig(ObservationManager.CONFIG_RETRIEVE_ENDDATE_FROM_SESSION)).booleanValue() ) {
				        ...
				        this.endTime.setTime(endDate.get(Calendar.HOUR_OF_DAY),
	...
	private void loadSchemaElement() {
        ...
        if( this.observation.getEyepiece() != null ) {
        	...
        	if( this.observation.getEyepiece().isZoomEyepiece() ) {
        		float afl = OpticsUtil.getActualFocalLength(this.observation.getScope(), this.observation.getLens(), this.observation.getMagnification());
	...
	private void createPanel() {		
        ...
        OMLabel Lend = new OMLabel(AbstractPanel.bundle.getString("panel.observation.label.end"), JLabel.RIGHT, false);        
        ...
        OMLabel LSeeing = new OMLabel(AbstractPanel.bundle.getString("panel.observation.label.seeing"), JLabel.RIGHT, false);        
        ...
        OMLabel Lsqm = new OMLabel(AbstractPanel.bundle.getString("panel.observation.label.sqm"), JLabel.RIGHT, false);    
        ...
        this.sqmValue = new SurfaceBrightnessContainer(null, true, new String[] {SurfaceBrightness.MAGS_SQR_ARC_MIN, SurfaceBrightness.MAGS_SQR_ARC_SEC});       
        ...
        JScrollPane imageContainerScroll = new JScrollPane(this.imageContainer, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);      
	...
	private void fillLists(boolean refill) {
		...
		for(int i=0; i < observers.length; i++) {
			...
			if(   (currentValue != null)
			   && (currentValue.equals(observers[i].getDisplayName()) )
			   ) {
		...
		//	if( this.observation == null) {   // Only show non-catalog targets (on create new observation)
		//		if( targets[i].getObserver() != null ) {
		//			this.targetBox.addItem(targets[i]);		
		//		}				
	...
	private void clearCacheData() {
		...
		// this.sessionBox.selectEmptyItem();  Don' clear session, was we're called from session changed
