...
public class PopupMenuHandler implements ActionListener {
	...
	private int createType = SchemaElementConstants.NONE;   // In case of create, this indicates which type of SchemaElement needs to be created
	
	public PopupMenuHandler(ObservationManager om,
				            ISchemaElement se,
							int x,
							int y,
							byte operation,
		...
		// Save schemaElement type we found as we need it for the extensions popup menus again (see below)
		...
		if(   ((operation & PopupMenuHandler.CREATE_HTML) == PopupMenuHandler.CREATE_HTML)
		   && ((operation & PopupMenuHandler.CREATE_XML) == PopupMenuHandler.CREATE_XML)	
			...
			if(   ((operation & PopupMenuHandler.CREATE_HTML) == PopupMenuHandler.CREATE_HTML )
			 /*  && (   (element instanceof IObservation)
        ...
        if( e.getSource() instanceof JMenuItem ) {
          ...
          if( source.equals(this.edit) ) {
        	if( element instanceof IObservation ) {
        		...
        		ScopeDialog dialog = new ScopeDialog(this.observationManager,
                        							 (IScope)this.element);
        	} else if( element instanceof IEyepiece ) {
        		EyepieceDialog dialog = new EyepieceDialog(this.observationManager,
        												   (IEyepiece)this.element);
        	} else if( element instanceof IImager ) {
        		IImager imager = (IImager)element;        		
        		IImagerDialog dialog = (IImagerDialog)this.observationManager.getExtensionLoader().getSchemaUILoader().getSchemaElementDialog(imager.getXSIType(), SchemaElementConstants.IMAGER, imager, true);        		
        	} else if( element instanceof ISite ) {
        		SiteDialog dialog = new SiteDialog(this.observationManager,
        										   (ISite)this.element);
        	} else if( element instanceof IFilter ) {
        		FilterDialog dialog = new FilterDialog(this.observationManager,
        										       (IFilter)this.element);        		
        	} else if( element instanceof ISession ) {
        		SessionDialog dialog = new SessionDialog(this.observationManager,
						 								 (ISession)this.element);
        	} else if( element instanceof IObserver ) {
        		ObserverDialog dialog = new ObserverDialog(this.observationManager,
						 								   (IObserver)this.element);
        	} else if( element instanceof ILens ) {
        		LensDialog dialog = new LensDialog(this.observationManager,
						 						   (ILens)this.element);
        	}
          		...
          		switch( this.createType ) {
          		  	...
          		  	case SchemaElementConstants.OBSERVATION: {
          		  				...
          		  				while(   (dialog == null)
          		  					  || ((dialog != null) && (dialog.isCreateAdditionalObservation()))
          		  							...
          		  							dialog = new ObservationDialog(this.observationManager,
          		  														   null);
          		  							this.observationManager.update(dialog.getObservation());
          		  				}		
		                		break;
          		  	...
          		  	case SchemaElementConstants.IMAGER: {
          		  				ExtenableSchemaElementSelector is = new ExtenableSchemaElementSelector(this.observationManager, this.observationManager.getExtensionLoader().getSchemaUILoader(), SchemaElementConstants.IMAGER);
			            		if( is.getResult() ) {
			            			// Get Imager Dialog
          		  	...
          		  	case SchemaElementConstants.TARGET: {
		                        ExtenableSchemaElementSelector ts = new ExtenableSchemaElementSelector(this.observationManager, this.observationManager.getExtensionLoader().getSchemaUILoader(), SchemaElementConstants.TARGET);
		                		if( ts.getResult() ) {
		                			// Get TargetContainer
