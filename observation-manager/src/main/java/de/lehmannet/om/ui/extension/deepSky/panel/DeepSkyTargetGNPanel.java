...
public class DeepSkyTargetGNPanel extends AbstractPanel {
  ...
  private final PropertyResourceBundle bundle = (PropertyResourceBundle)ResourceBundle.getBundle("de.lehmannet.om.ui.extension.deepSky.DeepSky", Locale.getDefault());	
    ...
    public DeepSkyTargetGNPanel(ObservationManager om, 
    		                    ITarget target, 
		...
		if(    (target != null)
		   && !(target instanceof DeepSkyTargetGN) 
		   ) {
			throw new IllegalArgumentException("Passed ITarget must derive from de.lehmannet.om.extension.deepSky.DeepSkyTargetGN\n");
		}
	...
	private void createPanel() {	
	    ...
	    OMLabel LpositionAngle = new OMLabel(this.bundle.getString("panel.gn.label.posAngle"), JLabel.RIGHT, false);
