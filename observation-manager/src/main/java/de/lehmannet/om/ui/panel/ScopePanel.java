...
public class ScopePanel extends AbstractPanel implements ActionListener {
  ...
  private IScope scope = null;
    ...
    public ScopePanel(IScope scope, 
					  boolean editable) {
	...
	private IScope getScope(boolean update,
			                float aperture,
							String modelName) {
		...
		if(   (focalLength == null)
		   || ("".equals(focalLength.trim()))
		   ) {
			// Magnification has to be set
			if(   (magnification != null)
			   && !("".equals(magnification.trim()))
			   ) {			
					float ma = 0.0f;
					try {
    ...
    private void createPanel() {
        ...
        OMLabel LvendorName = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.vendor"), JLabel.RIGHT, false);
        ...
        if( super.isEditable() ) {
        	LfocalLength = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.focalLength") + "*", JLabel.RIGHT, true);	
        } else {
        	LfocalLength = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.focalLength"), JLabel.RIGHT, true);
        }        
        ...
        if( super.isEditable() ) {
        	Lmagnification = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.magnification") + "*", JLabel.RIGHT, true);	
        } else {
        	Lmagnification = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.magnification"), JLabel.RIGHT, true);
        }               
        ...
        OMLabel LtfoV = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.trueFoV"), JLabel.RIGHT, false);        
        ...
        if( !super.isEditable() ) {   // Only show in display mode
            if(	  (this.scope != null)
                    && (!Float.isNaN(this.scope.getMagnification())) 
                    ) {
        ...
        // Add hint on how to use focal length and magnification, when we're in edit mode
        if( super.isEditable() ) {
            ConstraintsBuilder.buildConstraints(constraints, 0, rowCounter++, 4, 1, 45, 80);
            OMLabel focalLengthMagnificationHint = new OMLabel(AbstractPanel.bundle.getString("panel.scope.label.focalLengthMagnificationHint"), true); 
