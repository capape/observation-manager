...
public class ObserverPanel extends AbstractPanel implements MouseListener, ActionListener {
  ...
  private IObserver observer = null;
    ...
    public ObserverPanel(IObserver observer, 
	     	             boolean editable) {
    ...
    public void actionPerformed(ActionEvent e) {
    	...
    	if( e.getSource().equals(this.addAccountRow) ) {
    		...
    		// This is strange...when deleting a row (which is not the last row in the table)
    ...
    private void createPanel() {
	    ...
	    OMLabel Lsurname = new OMLabel(AbstractPanel.bundle.getString("panel.observer.label.surname"), JLabel.RIGHT, true);
	    ...
	    if(   (this.observer == null) 
	       || (this.observer.getAccounts() == null)
	       ) {
	    ...
	    if( super.isEditable() ) {	// Only show buttons in edit mode
	        ...
	        ConstraintsBuilder.buildConstraints(constraints, 0, 6, 4, 1, 45, 100); 
	...
	private String[] getAccountBoxItems(HashMap accounts) {							
		...
		for(int i=0; i < accountArray.length; i++) {
			...
			if(   Observer.ACCOUNT_AAVSO.equals(accountArray[i])
			   || Observer.ACCOUNT_DEEPSKYLOG.equals(accountArray[i])
  ...
  private String[] accounts = new String[] {};
	...
	public HashMap getAllEntries() {
		...
		for(int i=0; i < this.accounts.length; i++  ) {
			account = (String)this.accounts[i];
...
class AccountListRenderer extends DefaultListCellRenderer {
  ...
  public static String EMPTY_LIST_ENTRY = AbstractPanel.bundle.getString("panel.observer.account.comboBox.enterValue");
