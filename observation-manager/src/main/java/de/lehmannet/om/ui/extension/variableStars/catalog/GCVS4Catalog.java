...
public class GCVS4Catalog implements ICatalog {
	...
	public GCVS4Catalog(File catalogDir, ObservationManager om) {
		...
		// Get catalog File
		this.catalogFile = new File(catalogDir.getAbsolutePath() + File.separator + "variableStars" + File.separator + "gcvs4.dat");
	...
	private int searchByName(String objectName) {
		...
		if( cStart == -1 ) {	// There is no space in given object name. This pattern does not match the GCVS4 catalog
	...
	private int seqentialSearch(int[] startEnd, String objectName) {
		...
		try {
			...
			boolean found = false;
			do {				
				pointerPos = GCVS4Catalog.CATALOG_LINE_SIZE * currentLine;	// Calculate read/write pointer position in bytes
	...
	private ITarget createTarget(int lineNumber) {
		...
		try {
			...
			// Star is listed as with different designation. Read new designation and parse line again
			String additionalDesignation = null;
			if(   (equPosition == null)
			   && !("".equals(otherDesignations))
			   && !(
					   otherDesignations.startsWith("HIP")
					|| ( ((byte)otherDesignations.charAt(0) >= 48) && ((byte)otherDesignations.charAt(0) < 58) )  // Starts with number
					)
			   ) {
			...
			if( additionalDesignation != null ) {
				target.setAliasNames(new String[] {(GCVS4Catalog.CATALOG_ABB + gcvsNumber), otherDesignations, additionalDesignation});
			} else {
	...
	private int compareStrings(String a, String b) {
		...
		// Designation of var stars starts with R until ZZ and then starts again from A until QZ		
		if(   (b.compareTo("R") >= 0)		
		   || (a.compareTo("R") >= 0)
		   ) {
