...
public class SkyChartClient implements IExtension, ActionListener {
	...
	static {
		try {
			SkyChartClient.UPDATE_URL = new URL(
					"http://observation.sourceforge.net/extension/skychart/update");
	...
	private PropertyResourceBundle bundle = (PropertyResourceBundle) ResourceBundle
			.getBundle("de.lehmannet.om.ui.extension.skychart.Skychart",
					Locale.getDefault());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() instanceof JMenuItem) { // Should always be the case
			...
			if ((source.equals(this.mainMoveTo))
					|| (source.equals(this.popupMoveTo))) {
				...
				if (se instanceof IObservation) {
					...
					this.om.createWarning(this.bundle
							.getString("skychart.move.wrongSchemaElementType"));
				}
			}
		}
	...
	public void moveSkychart(ITarget target) {
		...
		if (result) {
			...
			if (response == null) {
				...
				// Inform user
				this.om.createInfo(this.bundle.getString("skychart.move.ok")
						+ target.getDisplayName());
			} else if (!response.booleanValue()) {
				// Inform user
				this.om.createInfo(this.bundle
						.getString("skychart.move.failed"));
			}
		} else {
			// Inform user
		...
		try {
			...
			// Can't do much here.
			System.err
					.println("Unable to close socket to Skychart application.\n"
							+ ioe);
		}
	...
	public void moveSkychart(IObservation observation) {
		...
		/*
		 * if( observation.getSite() != null ) { commands = new String[1];
		 * commands[0] = this.createSiteCommand(observation.getSite()); response
		 * = this.sendData(socket, commands[0]); if( response == null ) {
		 * return; // Something went wrong. User is already informed, so cancel
		 * here. } }
		 */
		...
		// ---- Set target site
		boolean result = this.moveSkychartToTarget(socket,
	...
	private boolean moveSkychartToTarget(StarchartSocket socket, ITarget target) {
		...
		for (int i = 0; i < commands.length; i++) {
			responseString = this.sendDataWithServerResponse(socket,
					commands[i]);
			if (responseString == null) { // Communication error
				System.err
						.println("Set Target by position...Failed with error");
				return false;
			} else {
				...
				if ((ra != null) && (dec != null)) {
					...
					try {
						...
						if (this.om.isDebug()) {
							System.out
									.println("RA or DEC string is malformed: RA: "
											+ ra + "\tDEC: " + dec);
						}
						continue;
					}
					...
					if (raDiff > 0.5) {
						if (this.om.isDebug()) {
							System.out.println("Found wrong "
									+ target.getName()
									+ " as RA differs more than 0.5 degree");
						}
					...
					if (decDiff > 0.5) {
						if (this.om.isDebug()) {
							System.out.println("Found wrong "
									+ target.getName()
									+ " as DEC differs more than 0.5 degree");
						}
						continue; // This is most propably not the object we're
									// searching
					}
					...
					if (this.om.isDebug()) {
						System.out
								.println("Found "
										+ target.getName()
										+ " as RA and DEC do not differ more than 0.5 degree");
					}
					return true;
				}
			}
	...
	private Boolean sendData(StarchartSocket socket, String command) {
		...
		try {
			...
			if ((response != null)
					&& ((response
							.startsWith(StarchartSocket.SERVER_RESPONSE_OK)) || (response
							.endsWith(StarchartSocket.SERVER_RESPONSE_OK)))) {
			...
			om.createWarning(this.bundle
					.getString("skychart.communication.failed"));
	...
	private String sendDataWithServerResponse(StarchartSocket socket,
			String command) {
	...
	private StarchartSocket createSocket() {
		...
		// Get IP
		String ip = this.om.getConfiguration().getConfig(
				SkyChartPreferences.CONFIG_SERVER_IP_KEY);
		if ((ip == null) || ("".equals(ip.trim()))) {
			ip = SkyChartPreferences.SERVER_DEFAULT_IP;
		}

		// Get Port
		String s_port = this.om.getConfiguration().getConfig(
				SkyChartPreferences.CONFIG_SERVER_PORT_KEY);
		...
		try {
			...
			om.createWarning(this.bundle
					.getString("skychart.communication.failed.host"));
			System.err
					.println("Unable to reach Skychart application. Host unknown.\n"
							+ uhe);
		} catch (ConnectException ce) { // SkyChart is most probably not open.
										// So try to start it
			// Try to get application path
			// final String applicationPath =
			final String applicationPath = this.om.getConfiguration().getConfig(SkyChartPreferences.CONFIG_APPLICATION_PATH);
			if (applicationPath == null || "".equals(applicationPath.trim())) { // No
																				// application
																				// path
																				// specified
				om.createWarning(this.bundle
						.getString("skychart.application.start.nopath"));
				System.err
						.println("Unable to reach Skychart application and unable to start it as no application path is provided.\n"
								+ ce);
			} else { // Try to start it
				System.out
						.println("Unable to reach Skychart application. Try to launch it.");
				...
				Worker startApplication = new Worker() {
					...
					public void run() {
						try {
							...
							om.createWarning(SkyChartClient.this.bundle
									.getString("skychart.application.start.failed"));
							System.err
									.println("Unable to start Skychart application ("
											+ applicationPath + ").\n" + ioe);
							...
							om.createWarning(SkyChartClient.this.bundle
									.getString("skychart.application.start.failed"));
							System.err
									.println("Failed to start Skychart application ("
											+ applicationPath + ").\n" + e);
						}
					}
					...
					public byte getReturnType() {
						...
						return (p.exitValue() == 0) ? Worker.RETURN_TYPE_OK
								: Worker.RETURN_TYPE_ERROR;
				...
				new ProgressDialog(
						this.om, this.bundle.getString("skychart.application.start.title"),						
	...
	private String[] createSEARCHCommands(ITarget target) {
		...
		String[] commands;
		if ((target.getAliasNames() != null)
				&& (target.getAliasNames().length > 0)) {
	...
	private String createSiteCommand(ISite site) {
		...
		EquPosition ep = new EquPosition(new Angle(0, Angle.DEGREE),
				site.getLatitude());
	...
	private String createDateCommand(Calendar date) {
		...
		return "SETDATE "
				+ dateString.substring(0, dateString.lastIndexOf("T") + 9);
	...
	private void initMenus() {
		...
		this.mainMoveTo = new JMenuItem(
				this.bundle.getString("skychart.menu.moveSkychart"));
		...
		this.popupMoveTo = new JMenuItem(
				this.bundle.getString("skychart.menu.moveSkychart"));
	...
	public PopupMenuExtension getPopupMenu() {
		...
		PopupMenuExtension pme = new PopupMenuExtension(new int[] {
				SchemaElementConstants.OBSERVATION,
				SchemaElementConstants.TARGET }, this.popupMenu);
	...
	public void reloadLanguage() {
		...
		this.bundle = (PropertyResourceBundle) ResourceBundle.getBundle(
				"de.lehmannet.om.ui.extension.skychart.Skychart",
				Locale.getDefault());
		this.initMenus();
...
class StarchartSocket extends Socket {
	...
	public StarchartSocket(String ip, int port, boolean verbose)
			throws IOException {
		...
		if (this.verbose) {
			System.out.println("Socket creation response from Skychart: "
					+ response);
		}
	...
	public String send(String command) throws IOException {
		...
		if (this.verbose) {
			...
			for (int i = 0; i < lfB.length; i++) {
				byteString = byteString + " " + (byte) lfB[i];
			}
		...
		do {
			...
			index++;
				...
				&& (r.indexOf(StarchartSocket.SERVER_RESPONSE_NOTFOUND) == -1) && (r
				.indexOf(StarchartSocket.SERVER_RESPONSE_FAILED) == -1))
				&& (index <= 3) // Wait for 3 responses for a OK or Failure from
								// Skychart
		);
