...
public class AAVSOVisualSerializer implements ISerializer {
	...
	// List of variable star observations (IObservation) which should be written into file
	...
	/**
	 * Constructor<br>
	 * @param softwareName Name and version of software used to create the format
	 * @param variableStarObservations List of IObservation elements for 
	 * serialization
	 */	
	public AAVSOVisualSerializer(String softwareName,
            					 List variableStarObservations) {
	...
	public int serialize(OutputStream stream) throws Exception {
		...
		while( iterator.hasNext() ) {
			...
			if( !(finding instanceof FindingVariableStar) ) {
				String message = "Finding must be of type de.lehmannet.om.extension.variableStars.FindingVariableStar for observaton: " + currentObservation;
			...
			// Write observer AAVSO initals to stream (in case the observer initials changed since last loop)
			...
			// Write comparism star data (only first and second here. Rest goes into comments)
			...
			if( comparismStars.size() > 2 ) {
				...
				while( compStarItertor.hasNext() ) {  
					notes = notes + (String)compStarItertor.next() + ",";		// This delimiter must be != AAVSOVisualSerializer.DELIMITER
				}
				notes = notes + " - ";
			}
            ...
            //GMB - escaping non-ASCII chars to html4 entities and replaces CR LF with dashes
            String rawNotes = "";
